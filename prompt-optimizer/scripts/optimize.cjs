...
const { validateFilePath, requireArgs } = require('@agent/core/validators');
...
runSkill('prompt-optimizer', () => {
  ...
  if (descLength > 200) {
    suggestions.push(
      'Shorten the description to 200 characters or less. Move details to the body.'
    );
  }
  ...
  if (!hasClarityIndicators) {
    suggestions.push(
      `Use more precise language. Include words like: ${CLARITY_INDICATORS.slice(0, 5).join(', ')}.`
    );
  }
  ...
  if (!noVagueWords) {
    suggestions.push(
      `Replace vague words (${vagueFound.join(', ')}) with specific, measurable terms.`
    );
  }
  ...
  if (!hasKnowledgeProtocol) {
    suggestions.push(
      'Add a "## Knowledge Protocol" section describing data handling and privacy tiers.'
    );
  }
  ...
  if (!hasExamples) {
    suggestions.push(
      'Add concrete usage examples (code blocks or quoted commands) to help users understand how to invoke the skill.'
    );
  }
  ...
  if (!sufficientContent) {
    suggestions.push(
      'Add more detail to the SKILL.md body. Aim for at least 100 characters of descriptive content.'
    );
  }
