...
const MODEL_CATALOG = [
  ...
  },
  {
    ...
    costPer1kTokens: 0.005,
    maxContext: 128000,
    strengths: ['general', 'multimodal', 'function-calling'],
    latencyMs: 1000,
    capabilities: ['text', 'code', 'analysis', 'multimodal', 'function-calling'],
  },
  {
    id: 'gpt-4o-mini',
    provider: 'openai',
    ...
    maxContext: 128000,
    strengths: ['speed', 'cost', 'function-calling'],
    latencyMs: 300,
    capabilities: ['text', 'code', 'analysis', 'function-calling'],
  },
  {
    id: 'llama-3.1-70b',
    provider: 'local',
    ...
    costPer1kTokens: 0,
    maxContext: 128000,
    strengths: ['privacy', 'cost', 'customizable'],
    latencyMs: 500,
    capabilities: ['text', 'code', 'analysis'],
  },
  {
...
function analyzeTaskComplexity(content) {
  ...
  const complexitySignals = {
    codeGeneration:
      /(?:implement|write|create|build|develop)\s+(?:a |an |the )?(?:function|class|module|service|api)/i.test(
        content
      ),
    reasoning: /(?:why|explain|analyze|compare|evaluate|design|architect)/i.test(content),
    creative:
      /(?:draft|write|compose|create|generate)\s+(?:a |an |the )?(?:story|blog|email|proposal|narrative)/i.test(
        content
      ),
  ...
  if (complexitySignals.codeGeneration) {
    hardness = 'medium';
    requiredCapabilities.push('code');
  }
  if (complexitySignals.reasoning) {
    hardness = 'high';
    requiredCapabilities.push('reasoning');
  }
  if (complexitySignals.creative) {
    requiredCapabilities.push('creative');
  }
  if (complexitySignals.multimodal) {
    requiredCapabilities.push('multimodal');
  }
  if (complexitySignals.security) {
    hardness = 'high';
    requiredCapabilities.push('reasoning');
  }
  if (complexitySignals.longContext) {
    hardness = hardness === 'low' ? 'medium' : hardness;
  }
  if (complexitySignals.simple) {
    hardness = 'low';
  }
...
function selectModel(complexity, budget) {
  ...
  if (candidates.length === 0) {
    return {
      primary: MODEL_CATALOG.find((m) => m.tier === 'premium'),
      fallbacks: [],
      reason: 'No model matches all requirements; falling back to premium',
    };
  }
  ...
  const scored = candidates.map((m) => {
    ...
    if (complexity.hardness === 'high')
      score += m.tier === 'premium' ? 10 : m.tier === 'balanced' ? 5 : 0;
...
runSkill('ai-model-orchestrator', () => {
  ...
  const result = {
    ...
    fallbackModels: selection.fallbacks.map((m) => ({
      id: m.id,
      provider: m.provider,
      tier: m.tier,
    })),
