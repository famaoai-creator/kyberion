...
function simulateAction(action, folder, query) {
  const results = {
    status: {
      connected: false,
      mode: 'dry-run',
      message: 'Box API not called (dry-run mode). Provide config and disable dry-run to connect.',
    },
    list: {
      folderId: folder,
      items: [
...
runSkill('box-connector', () => {
  ...
  if (!isDryRun && !config.found)
    throw new Error(
      'Box config not found. Create knowledge/personal/box_config.json with JWT credentials.'
    );
  ...
  const result = {
    ...
    result: actionResult,
    recommendations: !config.found
      ? [
          'Create Box JWT config at knowledge/personal/box_config.json',
