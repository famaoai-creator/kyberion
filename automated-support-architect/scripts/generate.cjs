...
const argv = createStandardYargs()
  ...
  .option('type', {
    ...
    type: 'string',
    default: 'faq',
    choices: ['faq', 'troubleshoot', 'chatbot', 'all'],
...
function extractFAQs(dir) {
  ...
  for (const err of errorPatterns) {
    faqs.push({
      q: `What does ${err} mean?`,
      a: getErrorExplanation(err),
...
function getErrorExplanation(code) {
  const explanations = {
    ...
    ECONNREFUSED:
      'Connection refused. The target service may not be running. Check if it is started.',
  };
...
function generateTroubleshoot(dir) {
  ...
  if (fs.existsSync(pkgPath)) {
    ...
    guides.push({
      issue: 'Tests failing',
      steps: [
        'Run `npm test` to see failure details',
        'Check for environment-specific issues',
        'Ensure test data/fixtures are present',
      ],
      category: 'testing',
    });
  }
...
function generateChatbotKB(faqs, troubleshoot) {
  ...
  for (const faq of faqs)
    intents.push({ intent: faq.q.replace(/[?]/g, ''), response: faq.a, category: 'faq' });
  for (const guide of troubleshoot)
    intents.push({
      intent: guide.issue,
      response: guide.steps.join('; '),
...
runSkill('automated-support-architect', () => {
  ...
  const chatbot =
    argv.type === 'chatbot' || argv.type === 'all' ? generateChatbotKB(faqs, troubleshoot) : null;
  const result = {
    directory: targetDir,
    type: argv.type,
    faqs: argv.type === 'faq' || argv.type === 'all' ? faqs : undefined,
    troubleshootingGuides:
      argv.type === 'troubleshoot' || argv.type === 'all' ? troubleshoot : undefined,
