...
const IP_INDICATORS = [
  {
    pattern: /(?:algorithm|heuristic|model|neural|machine.learning|deep.learning)/gi,
    category: 'Algorithm/Model',
    patentable: true,
  },
  {
    pattern: /(?:patent|trademark|copyright|intellectual.property)/gi,
    category: 'IP Reference',
    patentable: false,
  },
  {
    pattern: /(?:proprietary|trade.secret|confidential|novel)/gi,
    category: 'Trade Secret',
    patentable: false,
  },
  {
    pattern: /(?:unique|innovative|first.of.its.kind|breakthrough)/gi,
    category: 'Innovation Claim',
    patentable: true,
  },
  {
    pattern: /(?:protocol|specification|standard|format)/gi,
...
function scanForIP(dir) {
  ...
  for (const full of allFiles) {
    if (!['.js', '.cjs', '.ts', '.py', '.go', '.rs', '.java', '.md'].includes(path.extname(full)))
      continue;
    try {
...
runSkill('ip-strategist', () => {
  ...
  const result = {
    ...
    recommendations: [
      !license.protected
        ? '[critical] No LICENSE file - IP is unprotected'
        : `License: ${license.license}`,
      patentable.length > 0
        ? `[high] ${patentable.length} potentially patentable items found - consider IP review`
        : 'No patentable algorithms detected',
    ],
  };
