...
describe('MissionIntelligence helpers', () => {
  ...
  it('builds a clear confirmation prompt for risky actions', () => {
    const prompt = buildDangerousActionPrompt('mission MSN-1', 'restart runtime', false);

    expect(prompt.title).toBe('mission MSN-1 · restart runtime');
    expect(prompt.detail).toContain('may not be fully reversible');
    expect(prompt.confirmLabel).toBe('restart runtime');
