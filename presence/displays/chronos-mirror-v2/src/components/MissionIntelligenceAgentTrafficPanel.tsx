  ...
  return (
    <section className="grid gap-4">
            ...
            filteredAgentMessages.map((message, index) => (
              ...
              >
                <div className="flex flex-wrap items-center gap-2">
                  <div
                    className={`rounded-full border px-2 py-1 text-[11px] ${messageToneClass(message.tone)}`}
                  >
                  ...
                  {message.teamRole && (
                    <div className="rounded-full border kb-border-subtle kb-surface-raised/5 px-2 py-1 text-[11px] kb-text-muted">
                      {message.teamRole}
                    </div>
                  )}
                  {message.missionId && (
                    <div className="text-[11px] kb-text-muted">{message.missionId}</div>
                  )}
                  <div className="ml-auto text-[11px] font-mono kb-text-muted">
                ...
                <div className="mt-2 flex flex-wrap gap-3 text-[11px] kb-text-muted">
                  <span>
      ...
      <div ref={missionThreadPanelRef}>
        ...
        >
          <div className="mb-3 flex flex-wrap items-center gap-3 text-[11px] kb-text-muted">
            <span>
              {effectiveMissionId
            ...
            <span className="rounded-full border kb-border-subtle kb-surface-raised/5 px-2 py-1 text-[11px] kb-text-muted">
              {missionPinStatusLabel}
            </span>
            {effectiveMissionId ? (
              <button
                ...
                className="rounded-full border kb-border-accent kb-surface-accent px-2 py-1 text-[11px] kb-text-accent transition hover:kb-surface-accent"
              >
            ...
            {!effectiveMissionId || missionThread.length === 0 ? (
              ...
              missionThread.map((entry, index) => (
                ...
                >
                  <div className="flex flex-wrap items-center gap-2">
                    <div
                      className={`rounded-full border px-2 py-1 text-[11px] ${messageToneClass(entry.tone)}`}
                    >
                      {messageTypeLabel(entry.type)}
                    </div>
                    <div className="text-[11px] font-semibold kb-text-secondary">{entry.label}</div>
                    {entry.teamRole && (
                      <div className="rounded-full border kb-border-subtle kb-surface-raised/5 px-2 py-1 text-[11px] kb-text-muted">
                        {entry.teamRole}
                      </div>
        ...
        <div className="space-y-3">
          {filteredA2AHandoffs.length === 0 ? (
            ...
            filteredA2AHandoffs.map((handoff, index) => (
              ...
              >
                <div className="flex flex-wrap items-center gap-2">
                  <div className="rounded-full border kb-border-accent kb-surface-accent px-2 py-1 text-[11px] kb-text-accent">
                    a2a handoff
                  </div>
                  <div className="text-[11px] font-semibold kb-text-secondary">
