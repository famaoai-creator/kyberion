...
describe('TraceViewer helpers', () => {
  ...
  it('keeps a short most-recent-first trace focus history', () => {
    ...
    expect(buildTraceFocusHistory(['trace-a', 'trace-b'], 'trace-a', 3)).toEqual([
      'trace-a',
      'trace-b',
    ]);
  });
  ...
  it('loads trace viewer prefs with raw panel state', () => {
    ...
    expect(loadTraceViewerPrefs('[]')).toBeNull();
    expect(loadTraceViewerPrefs(JSON.stringify({ filters: 'not-an-object' }))).toEqual({
      filters: {
        status: 'all',
        missionId: '',
        pipelineId: '',
