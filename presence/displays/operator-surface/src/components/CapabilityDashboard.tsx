...
export default function CapabilityDashboard({ bundles, pins }: CapabilityDashboardProps) {
  return (
    <div style={{ marginTop: '40px', borderTop: '1px solid #2a2c33', paddingTop: '30px' }}>
      <h2 style={{ color: '#e4e6eb', marginBottom: '8px' }}>🤖 Capability & Extension Control Plane</h2>
      <p style={{ color: '#9aa0aa', marginTop: 0, fontSize: '14px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '30px', marginTop: '20px' }}>
            ...
            {bundles.map(bundle => (
              <div key={bundle.bundle_id} style={cardStyle(bundle.health)}>
                ...
                {bundle.dependencies && bundle.dependencies.length > 0 && (
                  <div style={{ marginTop: '12px', borderTop: '1px solid #2a2c33', paddingTop: '8px' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', marginTop: '4px' }}>
                      {bundle.dependencies.map(dep => (
                        <div key={dep.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px' }}>
                          <span style={{ color: '#9aa0aa' }}>{dep.id.replace('provider.runtime.', '')}</span>
        ...
        <div style={sectionBox}>
          ...
          {Object.keys(pins).length === 0 ? (
            <p style={{ color: '#9aa0aa', fontSize: '14px', fontStyle: 'italic' }}>
            ...
            <table style={tableStyle}>
              ...
              <tbody>
                {Object.entries(pins).map(([key, pin]: [string, any]) => (
                  <tr key={key} style={{ borderBottom: '1px solid #1a1c22' }}>
                    ...
                    <td style={tdStyle}><span style={{ color: '#9aa0aa', fontSize: '12px' }}>{new Date(pin.pinnedAt).toLocaleString()}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
...
const healthBadge = (health: 'active' | 'degraded' | 'inactive'): React.CSSProperties => {
  const colors = {
    active: { bg: '#13351f', text: '#56d364' },
    degraded: { bg: '#352e13', text: '#e3b341' },
    inactive: { bg: '#351313', text: '#f85149' }
  };
  return {
