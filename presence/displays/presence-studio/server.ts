...
presenceStudioData.app.get('/api/training/catalog', (_req, res) => {
  try {
    res.setHeader('Cache-Control', 'no-store');
...
presenceStudioData.app.get('/api/training/progress', (req, res) => {
  try {
    const viewer = resolvePresenceStudioViewerContext(req);
...
presenceStudioData.app.post('/api/training/progress', (req, res) => {
  try {
    const viewer = resolvePresenceStudioViewerContext(req);
...
presenceStudioData.app.get('/api/training/assignments', (req, res) => {
  try {
    const viewer = resolvePresenceStudioViewerContext(req);
...
presenceStudioData.app.post('/api/training/assignments', (req, res) => {
  try {
    const viewer = resolvePresenceStudioViewerContext(req);
    requirePresenceStudioLocalAdmin(viewer);
    const tenant = viewer.tenantSlugs === 'all' ? '' : viewer.tenantSlugs[0];
    ...
    return res.json({ ok: true, assignments: upsertTrainingAssignment(tenant, memberId, trackId) });
  } catch (error) {
