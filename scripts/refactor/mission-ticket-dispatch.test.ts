...
import { clearWorkCoordinationStore, setWorkCoordinationNamespace, listWorkItems, pathResolver, safeExistsSync, safeMkdir, safeReadFile, safeRmSync, safeWriteFile } from '@agent/core';
...
describe('mission ticket dispatch', () => {
  it('registers NEXT_TASKS as local work items and exports ticket payloads', async () => {
    ...
    const githubPayload = JSON.parse(safeReadFile(`${missionPath}/coordination/tickets/github/task-1.json`, { encoding: 'utf8' }) as string);
    const jiraPayload = JSON.parse(safeReadFile(`${missionPath}/coordination/tickets/jira/task-1.json`, { encoding: 'utf8' }) as string);
  ...
  it('rejects tasks without an assigned owner or enough detail', async () => {
    ...
    expect(manifest.records[0].notes).toEqual(expect.arrayContaining([
