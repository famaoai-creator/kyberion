...
function parseSections(
  markdown: string
): Map<string, { body: string[]; subsections: Map<string, string[]> }> {
...
function pickPaletteValue(
  palette: PaletteToken[],
  predicates: Array<(token: PaletteToken) => boolean>
): string | undefined {
...
function inferTone(imported: ImportedDesign): string {
  const haystack = [imported.category, imported.description, imported.visual_theme]
    .join(' ')
    .toLowerCase();
...
function importDesignMd(
  dirPath: string,
  collectionMeta: Map<string, CollectionEntry>
): ImportedDesign {
  ...
  const palette = colorsSection
    ? [...colorsSection.subsections.entries()].flatMap(([group, lines]) =>
        parsePalette(lines, group)
      )
    : [];
  const fonts = typographySection?.subsections.get('Font Family')
    ? parseFonts(typographySection.subsections.get('Font Family')!)
    : {};
  const componentSections = componentsSection ? [...componentsSection.subsections.keys()] : [];
  ...
  return {
    ...
    prompt_guide: extractBullets([
      ...
      ...[
        ...((agentGuideSection?.subsections.values() || []) as IterableIterator<string[]>),
      ].flat(),
    ]),
...
async function renderImportDesignMdCatalog(context: ScriptContext): Promise<GeneratedFile[]> {
  const argv = await createStandardYargs([
    ...
    .option('source', {
      ...
      description: 'Directory containing awesome-design-md/design-md/*/DESIGN.md',
    })
    .option('readme', {
      type: 'string',
      default: README_PATH,
      description: 'awesome-design-md README path',
    })
    .parse();
  ...
  const imported = walkDesignMdDirs(sourceDir).map((dirPath) =>
    importDesignMd(dirPath, collectionMeta)
  );
