...
runTest(
  'data-transformer',
  `node data-transformer/scripts/transform.cjs -i "${csvFile}" -t json -o "${jsonFile}"`,
  jsonFile
);
...
runTest(
  'template-renderer',
  `node template-renderer/scripts/render.cjs -t "${templateFile}" -d "${dataObjFile}" -o "${path.join(workDir, 'rendered.txt')}"`
);

// 3. Data Collector
runTest(
  'data-collector',
  `node data-collector/scripts/collect.cjs --url "${csvFile}" --out "${path.join(workDir, 'collected')}"`
);

// 4. Context Injector
runTest(
  'context-injector',
  `node context-injector/scripts/inject.cjs -d "${dataObjFile}" -k "${mdFile}" -o "${path.join(workDir, 'injected.json')}"`
);

// 5. Glossary Resolver
runTest(
  'glossary-resolver',
  `node glossary-resolver/scripts/resolve.cjs -i "${mdFile}" -g "${glossaryFile}" -o "${path.join(workDir, 'resolved.txt')}"`
);

// 6. Word Artisan
runTest(
  'word-artisan',
  `node word-artisan/scripts/convert.cjs -i "${mdFile}" -o "${path.join(workDir, 'out.docx')}"`
);

// 7. PDF Composer
runTest(
  'pdf-composer',
  `node pdf-composer/scripts/compose.cjs -i "${mdFile}" -o "${path.join(workDir, 'out.pdf')}"`
);

// 8. HTML Reporter
runTest(
  'html-reporter',
  `node html-reporter/scripts/report.cjs -i "${mdFile}" -o "${path.join(workDir, 'out.html')}"`
);

// 9. Sequence Mapper
runTest(
  'sequence-mapper',
  `node sequence-mapper/scripts/map.cjs -i "${codeFile}" -o "${path.join(workDir, 'seq.mmd')}"`
);

// 10. Dependency Grapher
runTest(
  'dependency-grapher',
  `node dependency-grapher/scripts/graph.cjs -d "." -o "${path.join(workDir, 'deps.mmd')}"`
);

// 11. Diff Visualizer
runTest(
  'diff-visualizer',
  `node diff-visualizer/scripts/diff.cjs -a "${mdFile}" -b "${mdFile}" -o "${path.join(workDir, 'diff.patch')}"`
);

// 12. API Doc Generator
runTest(
  'api-doc-generator',
  `echo '{"openapi":"3.0.0","info":{"title":"Test","version":"1.0"}}' > "${path.join(workDir, 'api.json')}" && node api-doc-generator/scripts/generate.cjs -i "${path.join(workDir, 'api.json')}" -o "${path.join(workDir, 'api.md')}"`
);
...
runTest(
  'schema-validator',
  `node schema-validator/scripts/validate.cjs -i "${dataObjFile}" -s "${schemaFile}"`
);
...
runTest(
  'api-fetcher',
  `node api-fetcher/scripts/fetch.cjs --url "https://jsonplaceholder.typicode.com/todos/1" --out "${path.join(workDir, 'api_out.json')}"`
);
...
setTimeout(() => {
  runTest(
    'db-extractor',
    `node db-extractor/scripts/extract.cjs --db "${dbFile}" --out "${path.join(workDir, 'db_schema.json')}"`,
    path.join(workDir, 'db_schema.json')
  );
  ...
  runTest(
    'audio-transcriber',
    `node audio-transcriber/scripts/transcribe.cjs --file "${dummyAudio}" --key "dummy"`,
    null,
    true
  );
