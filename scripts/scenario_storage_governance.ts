...
export async function main(print: Print = () => undefined): Promise<void> {
  activePrint = print;
  try {
    ...
    if (safeExistsSync(sharedLogsProcess('scenario-daemon.log'))) {
      ...
      for (const line of lines) {
        ...
        info(
          `  [${entry.level}] ${entry.msg}${entry.meta ? ' ' + JSON.stringify(entry.meta) : ''}`
        );
      }
    } else {
    ...
    info(
      `  vault expired: ${report.expiredDataVault} (just added scenario entries — all within TTL)`
    );
...
export const runScenarioStorageGovernance = defineScript({
  name: 'scenario:storage-governance',
  flags: [],
  run: ({ print }) => main(print),
});

if (
  isDirectScript(import.meta.url, 'scenario_storage_governance.ts') ||
  isDirectScript(import.meta.url, 'scenario_storage_governance.js')
)
  void runScenarioStorageGovernance();
