 ...
 * knowledge/product/governance/health-thresholds.json) and prints the
 * report. Non-green verdicts have already been escalated through the AO-03
 * ops-alert sink by the time this exits; the exit code stays 0 so the
 * hourly schedule never spams pipeline failures on top of the alert.
 *
 * Scheduled via pipelines/health-degradation-watch.json (hourly).
 */

import { logger, runDegradationWatch } from '@agent/core';
