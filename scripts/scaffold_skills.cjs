...
skills.forEach(skill => {
    ...
    const title = skill.name.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
...
`;
    ...
    fs.writeFileSync(path.join(skillDir, 'package.json'), JSON.stringify(packageJsonContent, null, 2));
