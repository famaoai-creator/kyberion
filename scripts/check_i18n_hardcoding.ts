...
// must stay) at zero.
//
// Surface UI unification (final round): the static surfaces served as plain
// HTML + browser JS (presence-studio, computer-surface) are ratcheted too.
// Their copy belongs in the vocabulary catalog like everything else; JSON
// (gallery fixtures, message bundles) is data and never scanned, and a file
// whose header says it is generated is skipped (see isGeneratedFileText).
const JS_SCAN_PATTERNS = [
  /^libs\/shared-ui\/(?:src|vanilla)\/.*\.[cm]?js$/u,
  /^presence\/displays\/presence-studio\/static\/ui-gallery[^/]*\.js$/u,
  /^presence\/displays\/(?:presence-studio|computer-surface)\/static\/[^/]+\.js$/u,
];
const HTML_SCAN_PATTERNS = [
  /^presence\/displays\/(?:presence-studio|computer-surface)\/static\/[^/]+\.html$/u,
];

export function isScannedHtmlFile(repoRelativePath: string): boolean {
  return HTML_SCAN_PATTERNS.some((pattern) => pattern.test(repoRelativePath));
}

export function isScannedSourceFile(repoRelativePath: string): boolean {
  if (/\.(?:ts|tsx)$/u.test(repoRelativePath)) return true;
  if (isScannedHtmlFile(repoRelativePath)) return true;
  return JS_SCAN_PATTERNS.some((pattern) => pattern.test(repoRelativePath));
}

// A generated file announces itself in its first lines ("GENERATED by ...",
// "Generated by ... do not edit"); its copy is owned by the generator input.
const GENERATED_HEADER_PATTERN = /\bGENERATED\b|\bGenerated by\b[^\n]*\bdo not edit\b/u;

export function isGeneratedFileText(text: string): boolean {
  return GENERATED_HEADER_PATTERN.test(text.split('\n').slice(0, 5).join('\n'));
}
...
// HTML opt-out: "<!-- i18n-exempt: <reason> -->" on the same line as (or the
// line above) a flagged text node / attribute value.
const HTML_EXEMPT_COMMENT_PATTERN = /<!--\s*i18n-exempt:\s*([^>]*?)\s*-->/u;

/** Replace every character except newlines, so offsets and line numbers stay valid. */
function blankOut(text: string): string {
  return text.replace(/[^\n]/gu, ' ');
}

/**
 * Scans a static HTML page: inline `<script>` bodies are scanned as JS (same
 * rules as .js files); in the remaining markup (comments and `<style>`
 * removed) every kana-bearing text node and attribute value counts once.
 */
export function scanHtmlForKanaLiterals(text: string, repoRelativePath: string): FileScanResult {
  let count = 0;
  let exemptions = 0;
  ...
  const isExempt = (lineIndex: number) =>
    [originalLines[lineIndex] || '', lineIndex > 0 ? originalLines[lineIndex - 1] || '' : ''].some(
      (line) => {
        const match = HTML_EXEMPT_COMMENT_PATTERN.exec(line);
        return Boolean(match && match[1].trim().length > 0);
      }
    );
  const visit = (value: string, offset: number) => {
    if (!KANA_PATTERN.test(value)) return;
    if (isExempt(lineOf(offset))) {
      exemptions += 1;
      return;
    }
    count += 1;
  ...
  // Text nodes: runs between a tag end and the next tag start.
  for (const match of markup.matchAll(/>([^<]+)</gu)) {
    visit(match[1], (match.index ?? 0) + 1);
  }
  // Attribute values inside tags.
  for (const tag of markup.matchAll(/<[a-zA-Z][^>]*>/gu)) {
    for (const attr of tag[0].matchAll(/=\s*(?:"([^"]*)"|'([^']*)')/gu)) {
      visit(attr[1] ?? attr[2] ?? '', (tag.index ?? 0) + (attr.index ?? 0));
    }
  }
  return { count, exemptions };
}
...
function scanTree(scanRoots: string[]): {
  ...
  for (const root of scanRoots) {
    ...
    for (const filePath of getAllFiles(root)) {
      ...
      if (!/\.(?:ts|tsx)$/u.test(repoRelativePath) && isGeneratedFileText(text)) continue;

      checkedFiles += 1;
      scannedFiles.add(repoRelativePath);
      const { count, exemptions } = isScannedHtmlFile(repoRelativePath)
        ? scanHtmlForKanaLiterals(text, repoRelativePath)
