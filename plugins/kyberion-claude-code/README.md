...
| **Document reading guard**  | `PreToolUse` hook (Bash, deny-only) + `kyberion-document-reading` skill | Blocks hand-rolled office / PDF extraction (unzip, pdftotext, python-docx, …) and points to `pnpm kyberion read`; every other command falls through to Claude Code's own permission flow. |
...
- v0.3: deny-only Bash gate (`PreToolUseShellDeny`, shell-command-policy denylist) and the `kyberion-document-reading` skill.
- Planned: richer mission-level summaries.
