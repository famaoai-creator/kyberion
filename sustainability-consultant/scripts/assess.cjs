...
function assessInfraEnergy(dir) {
  ...
  if (exists('docker-compose.yml') || exists('Dockerfile'))
    usage.push(ENERGY_FACTORS.dockerCompose);
  if (exists('k8s') || exists('kubernetes') || exists('helm'))
    usage.push(ENERGY_FACTORS.kubernetes);
  if (exists('.github/workflows') || exists('.gitlab-ci.yml'))
    usage.push(ENERGY_FACTORS.cicdPipeline);
  ...
  return {
    ...
    estimatedMonthlyCO2Kg: co2Kg,
    annualCO2Kg: Math.round(co2Kg * 12),
  };
}
    ...
    try {
      ...
      if (stat.size > 10 * 1024 * 1024)
        waste.push({
          ...
          size: stat.size,
          issue: 'Large file (>10MB) - consider compression or external storage',
        });
      if (/\.(log|tmp|bak|old|orig)$/.test(path.basename(full)))
        waste.push({
          file: path.relative(dir, full),
          size: stat.size,
          issue: 'Temporary/log file in repository',
        });
    } catch (_e) {}
  }
  return waste;
...
function generateRecommendations(energy, waste) {
  const recs = [];
  if (energy.estimatedMonthlyKwh > 200)
    recs.push({
      priority: 'high',
      action: 'Consider serverless or spot instances to reduce always-on compute',
      savings: '30-60% energy reduction',
    });
  if (energy.components.some((c) => c.label.includes('Kubernetes')))
    recs.push({
      priority: 'medium',
      action: 'Enable cluster autoscaling and use preemptible nodes',
      savings: '20-40% compute reduction',
    });
  if (waste.length > 0)
    recs.push({
      priority: 'medium',
      action: `Remove ${waste.length} wasteful files (${Math.round(waste.reduce((s, w) => s + w.size, 0) / 1024)} KB total)`,
      savings: 'Reduced storage footprint',
    });
  recs.push({
    priority: 'low',
    action: 'Choose green cloud regions with renewable energy',
    savings: 'Up to 80% carbon reduction',
  });
  return recs;
}
