...
function detectPrerequisites(dir) {
  ...
  if (
    fileExists(dir, 'docker-compose.yml') ||
    fileExists(dir, 'docker-compose.yaml') ||
    fileExists(dir, 'Dockerfile')
  ) {
...
function generateSetupSteps(dir) {
  ...
  if (fileExists(dir, 'requirements.txt')) {
    steps.push(
      `${steps.length + 1}. Install Python dependencies: \`pip install -r requirements.txt\``
    );
  }
