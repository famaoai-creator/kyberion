...
function audit() {
  ...
  try {
    ...
    repos.forEach((repo) => {
      ...
      // Classify
      if (
        name.includes('project_a-') ||
        name.includes('project_b_') ||
        name.includes('generic_whitelabel') ||
        name.includes('generic_banking_api')
      ) {
        ...
        name.includes('identity-provider-') ||
        name.includes('auth-logic') ||
        name.includes('auth-server') ||
        name.includes('secure-auth')
      ) {
        mapping['Auth Infrastructure'].push(repo);
      } else if (
        name.includes('transfer-') ||
        name.includes('digital-wallet-') ||
        name.includes('dlt-network-') ||
        name.includes('asset-token-')
      ) {
        mapping['Digital Assets'].push(repo);
      } else if (
        name.includes('cloud-platform-') ||
        name.includes('reliability-') ||
        name.includes('terraform-') ||
        name.includes('ansible-') ||
        name.includes('infra-ops-')
      ) {
