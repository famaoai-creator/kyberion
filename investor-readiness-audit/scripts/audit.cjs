...
function auditDataRoom(dir, stage) {
  ...
  return {
    ...
    totalItems,
    foundItems,
    completionPercent: Math.round((foundItems / totalItems) * 100),
  };
}
...
function assessRisks(audit, stage) {
  ...
  for (const cat of audit.results) {
    ...
    if (missing.length > 0) {
      ...
      risks.push({
        ...
        impact: `${missing.length} missing item(s) in ${cat.category}`,
      });
    }
  }
  if (audit.completionPercent < 50)
    risks.push({
      ...
      severity: 'critical',
      missing: [],
      impact: `Only ${audit.completionPercent}% ready for ${stage} - significant preparation needed`,
    });
  return risks;
}
