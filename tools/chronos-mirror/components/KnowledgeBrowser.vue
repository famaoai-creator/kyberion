...
const filteredFiles = computed(() => {
  ...
  return files.value.filter(
    (f) => f.title.toLowerCase().includes(query) || f.id.toLowerCase().includes(query)
  );
});
...
<template>
  <div
    class="flex h-[450px] border border-gray-700 rounded-lg overflow-hidden bg-gray-900 bg-opacity-50 shadow-2xl text-left"
  >
    <!-- サイドバー: ファイルリスト -->
    <div class="w-1/3 border-r border-gray-700 flex flex-col bg-black bg-opacity-20">
      ...
      <div class="flex-1 overflow-y-auto custom-scrollbar">
        <div
          ...
          :class="[
            'p-2 cursor-pointer text-[10px] border-b border-gray-800 hover:bg-blue-900 hover:bg-opacity-30 transition',
    ...
    <div class="w-2/3 p-6 overflow-y-auto custom-scrollbar bg-white bg-opacity-[0.02]">
      <div v-if="selectedFile" class="prose prose-sm prose-invert max-w-none">
        <div class="mb-4 pb-2 border-b border-gray-800 flex justify-between items-center">
          <span class="text-[10px] font-mono text-blue-400 uppercase tracking-widest">{{
      ...
      <div
        v-else
        class="h-full flex items-center justify-center text-gray-600 animate-pulse italic text-sm"
      >
...
.markdown-body h2 {
  @apply text-lg font-bold mt-6 mb-2 border-b border-gray-800 pb-1;
}
.markdown-body h3 {
  @apply text-base font-bold mt-4 mb-1 text-gray-200;
}
.markdown-body p {
  @apply mb-3 text-gray-300;
}
.markdown-body ul {
  @apply list-disc list-inside mb-3 ml-2;
}
.markdown-body code {
  @apply bg-gray-800 px-1 rounded text-pink-400 font-mono;
}
.markdown-body pre {
  @apply bg-black p-3 rounded mb-4 overflow-x-auto border border-gray-800;
}
