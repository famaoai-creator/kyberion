...
<template>
  ...
  >
    ...
    <div class="flex-1 overflow-y-auto space-y-4 pr-2 custom-scrollbar">
      <div
        v-for="msg in timeline.slice().reverse()"
        :key="msg.id"
        :class="[
          'p-3 rounded-lg border border-gray-800 transition-all',
          msg.status === 'thinking'
            ? 'bg-blue-900 bg-opacity-10 border-blue-900'
      ...
      >
          ...
          <span
            :class="[
              ...
              msg.status === 'complete'
                ...
                : 'bg-yellow-900 text-yellow-400 animate-pulse',
            ]"
          >
            {{ msg.status }}
          </span>
        ...
        <pre v-if="msg.result" class="text-[9px] text-gray-400 mt-2 whitespace-pre-wrap">{{
          msg.result
        }}</pre>
      ...
      <div
        ...
        class="h-full flex items-center justify-center text-gray-600 italic text-sm animate-pulse"
      >
