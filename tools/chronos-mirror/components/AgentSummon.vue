...
<template>
  <div class="p-4 border border-blue-500 rounded-lg bg-blue-900 bg-opacity-10 my-4 shadow-inner">
    ...
    <div
      v-if="result"
      class="mt-4 p-3 bg-black bg-opacity-40 rounded text-[10px] font-mono text-blue-300 overflow-y-auto max-h-40 border border-blue-900"
    >
      <pre>{{ result }}</pre>
    </div>
    ...
    <div
      v-if="error"
      class="mt-4 p-2 bg-red-900 bg-opacity-20 rounded text-[10px] text-red-400 font-mono italic border border-red-900"
    >
      Error: {{ error }}
    </div>
  </div>
