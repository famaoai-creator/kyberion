...
Kyberion's readiness check is `pnpm run doctor`. After `pnpm build`, `pnpm doctor` in this first-win block is intended to run the repo script; bare `pnpm doctor` without `run` is pnpm's own diagnostic.
