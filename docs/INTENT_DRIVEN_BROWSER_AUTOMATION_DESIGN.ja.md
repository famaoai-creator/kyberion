...
review_record:
  - reviewer: Antigravity (Ecosystem Architect)
    ...
    folded_in:
      [§4 基盤決定, §6.2 しきい値, §7-Layer③ SPA/iframe, §7-Layer⑤ MFA猶予Lease, §12 決定確定]
depends_on:
...
- **ミッション化の判定**：必須トリガーが1つ以上、または蓄積トリガーが2つ以上なら `scripts/mission_controller.ts` でミッションを開始する。該当しない場合は `work-scope-policy.json` に従い、最低限の execution shape を選ぶ。
