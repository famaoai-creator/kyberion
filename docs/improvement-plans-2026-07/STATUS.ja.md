...
| SA-05 | PARTIAL | 2026-07-12 再突合: secure-io parse 失敗の fail-open は 2026-07-04 に解消済み(誤解を招く「allowing by default」ログのみ残っていたため文言修正 + fail-closed 回帰テスト追加)。-y 破壊操作除外も ui.confirm({destructive}) として実装・テスト済み。残: policyEngine の操作種別拡張(network/shell/delegation/actuator dispatch のゲート接続と ring/delegation_depth/capability 条件の発火文脈)                                                                                                                |
...
| SA-05 | PARTIAL | 残: policyEngine 操作種別拡張のみ(secure-io fail-closed と -y 破壊除外は 2026-07-12 再突合で実装+テスト済みを確認)                                                                                                                                            |
