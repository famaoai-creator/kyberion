...
| AR-02 | PARTIAL | 2026-07-12: describeOps を pipeline 系7種 + ロングテール6種(process/terminal/secret/approval/agent/artifact、47 op)へ拡大。shared pool 命中 op は歴史的 kind を保存(step-type 推論不変)、他は従来 unknown(throw)だった純追加。残: 残余アクチュエータ(email/calendar/vision/voice/media 系ほか)と CAPABILITIES op 表の生成切替                                                                                                                                                                             |
