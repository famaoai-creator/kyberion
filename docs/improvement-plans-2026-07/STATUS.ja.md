...
| AO-02 | PARTIAL | 2026-07-11: scan/台帳/ルーブリック実装済み + §3.3 unit test + 適用フロー(propose 既定・--apply で backup→bump→install/typecheck/smoke→再スキャン→確定/ロールバック)。残: 定期スケジュール接続(AO-01)、透過依存 override、カナリア監視(Task 4 defer 再評価は 2026-07-11 実装済み) |
