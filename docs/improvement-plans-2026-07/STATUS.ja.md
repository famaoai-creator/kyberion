...
| IP-08 | PARTIAL | 2026-07-12: 空 catch triage 台帳完成(94箇所を a32/b49/c13 に分類、計画文書末尾)。(c) 実バグ根治: tier-guard の破損ポリシー fail-open を fail-closed 化 + 沈黙 catch 4箇所へ警告付与。(b) 49箇所へ logger.warn 全件付与(29ファイル、制御フロー不変)。console→logger 完了(9箇所置換、CLI/対話/logger 実装/ブラウザ文脈の除外一覧を文書化)。残: process.exit 除去(library コード) |
...
| ----- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| KM-01 | DONE    |                                                                                                                                                                                                                                          |
| KM-02 | PARTIAL | 2026-07-12: 非 Mac 実埋め込み経路(Gemini text-embedding-004、MLX→Gemini→hash の解決順、secureFetch/egress 統制下)。キャッシュ LRU 実装(usage sidecar + KYBERION_KI_CACHE_MAX_MB 予算、既定200MB)。残: before/after fixture、ランカー統合 |
| KM-03 | DONE    |                                                                                                                                                                                                                                          |
| KM-04 | DONE    |                                                                                                                                                                                                                                          |
