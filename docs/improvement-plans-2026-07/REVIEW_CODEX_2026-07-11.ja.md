...
| P1     | Windows capability 宣言を CI matrix と一致させる  | win32 宣言に対し ubuntu/macOS のみ検証          | IP-06、IP-03、2026-07-11 warn 観測4ラウンド完了: build/typecheck/contract-schemas は Windows 通過(quoting・schema-id の実バグ2件を修正)。test:core は 2,072/2,485(83%)通過、失敗135ファイルのインベントリ取得。残トリアージ(テストの win32 密閉化 vs 宣言縮小)は本インベントリ基準で別ワークストリーム化 |
