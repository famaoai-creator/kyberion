...
> **進捗(2026-07-07)**: `libs/core/src/pipeline-engine.ts` に汎用 `executeAdfSteps` を追加し、`file-actuator` の pipeline ループをそこへ委譲し始めた。共通 runner のパイロットとして nested control と context 解決の回帰テストを追加済み。
