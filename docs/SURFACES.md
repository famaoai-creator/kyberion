...
| --------------------- | ----------------------------------------------------------------------------------- | ---------------------------------------------- | ---- | ------------------------- | --------------------------------------------------------------------------- |
| **concierge(秘書室)** | **CEO秘書** — 依頼・承認・成果・例外                                                | 「私は今なにを判断すればよいか」               | 3050 | 承認/受領のみ             | `active-surfaces.json`(`pnpm surfaces:reconcile`)                           |
| **presence-studio**   | **相棒** — いっしょに作業するワークベンチ(音声・議事録・メール・クイックアクション) | 「いま一緒に何を進めるか」                     | 3031 | 実作業                    | `active-surfaces.json`                                                      |
| **chronos-mirror-v2** | **管制塔** — 実行状態の監視と介入(プラン確認→ミッション起動、承認、成果物レビュー)  | 「システムは何をしていて、どこに介入すべきか」 | 3000 | 介入                      | `pnpm chronos:dev` / `active-surfaces.json`                                 |
...
| `pnpm kyberion`                       | オペレータホームのサマリ(CLI)                             | 直接の変更操作はしない |
