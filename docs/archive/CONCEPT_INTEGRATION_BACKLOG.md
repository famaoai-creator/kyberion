...
- **構成**：
  ...
  - `dual-key-policy.md` ← origin/main 既存、ただし _role-switching governance_ で approval-gate と別関心
  - `execution-receipt-policy` ← 受領書 _フォーマット_ 規定、強制機構ではない
  - `approval-gate.ts` ← ローカル追加、**上記 3 コンポーネントを束ねる pre-execution enforcement glue**
- **判定**：approval-gate は重複ではなく _欠けていたグルー層_。保持。撤去対象なし。
