...
| UX-0-3 | first-time-user 向け readiness 圧縮           | 1, 4, 13, 15                      | `pnpm kyberion setup report`, `pnpm kyberion vital --format=json`, `libs/core/next-action.ts`                                                                               | readiness 出力が noise を畳み、must / should / nice と next action が 1〜3 個に絞られる                                                                                       |
...
| Command         | `pnpm run validate`, `pnpm test -- --suite core`, `pnpm run check:contract-semver`, golden scenario checks |
...
- P0-1 は `pnpm doctor` を baseline / reasoning の既定確認にし、meeting / voice / browser / audio の runtime 不足は `pnpm doctor -- --runtime meeting --mission <MISSION_ID>` で must / should / nice と domain label 付きで分類する。不足解消は同じ manifest を `pnpm env:bootstrap --manifest meeting-participation-runtime --apply` に渡す。
...
- P1-5 は `.github/workflows/cross-os.yml` の Ubuntu / macOS matrix に schema、core test、pipeline preview、shell independence に加えて `pnpm test -- --suite meeting-dry-run` を入れる。Dry-run は stub coordinator と browser-driver contract tests を実行し、実 meeting / OS audio device に依存せず consent gate・domain allowlist・URL redaction を固定する。
...
- P3-4 は `pipelines/ui-voice-browser-smoke.json` と `pnpm test -- --suite ui-voice-browser-smoke` を追加し、presence-studio の health / voice UI、`voice-hello`、`verify-session`、meeting consent gate を 1 つの smoke slice として固定する。
