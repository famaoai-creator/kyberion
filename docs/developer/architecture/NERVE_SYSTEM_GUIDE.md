...
長寿命の神経系プロセスは `knowledge/product/governance/surfaces/*.json` と `surface-runtime` で宣言的に管理します。`active-surfaces.json` は互換 snapshot です。単発の補助プロセスは `process-actuator` に委譲します。
...
1.  **Shield Layer**: durable runtime の定義変更は `knowledge/product/governance/surfaces/*.json` と review を通してください。`active-surfaces.json` は generated compatibility snapshot です。
...
_Created by Infrastructure Sentinel for the Kyberion Sovereign Ecosystem._
