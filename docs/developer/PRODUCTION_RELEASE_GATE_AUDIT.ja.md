...
| G3: personal / confidential / public と tenant / group scope                   | `pnpm test -- --suite core` passed 206 files / 1171 tests, including tier-guard and governance regressions.                                                                                                                                                                                                  | Pass                          |
| G4: meeting / voice / browser consent safety                                   | `pnpm test -- --suite ui-voice-browser-smoke`, `pnpm test -- --suite meeting-dry-run`, and `libs/actuators/meeting-actuator/src/index.test.ts` cover consent-denied and consent-granted paths.                                                                                                               | Pass                          |
| G5: doctor / bootstrap detects runtime capability gaps and next action         | `pnpm run doctor` reports all required baseline capabilities satisfied. `pnpm doctor -- --runtime meeting --mission P3-5-REFERENCE-DRIFT-AUDIT` fails closed for missing BlackHole / voice consent and prints `pnpm env:bootstrap --manifest meeting-participation-runtime --apply` plus consent next steps. | Pass for detection / guidance |
| G6: representative scenarios are repeatable by golden / contract / smoke tests | `pnpm run validate`, `pnpm check -- --scope full --only golden`, `pnpm test -- --suite core`, `pnpm test -- --suite ui-voice-browser-smoke`, and actual `verify-session` execution passed.                                                                                                                   | Pass                          |
...
| Backlog                                | Evidence                                                                                                                                                                                                                               |
| -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P0-1 doctor / bootstrap                | `scripts/run_doctor.ts`, `scripts/environment-doctor.ts`, `knowledge/product/governance/environment-manifests/meeting-participation-runtime.json`, targeted tests, `pnpm run doctor`, `pnpm doctor -- --runtime meeting --mission ...` |
...
| P0-5 pipeline shell independence       | `scripts/check_pipeline_shell_independence.ts`, `pnpm run check -- --scope full --only pipeline-shell-independence`                                                                                                                    |
| P0-6 golden scenario catalog schema    | `scripts/check_contract_schemas.ts`, governance catalog tests, `pnpm run check -- --scope full --only contract-schemas`                                                                                                                |
...
| P1-4 browser participation safety      | `libs/core/meeting-participation-coordinator.ts`, meeting browser-driver tests, `test -- --suite meeting-dry-run`                                                                                                                      |
...
| P3-4 UI / voice / browser smoke        | `pipelines/ui-voice-browser-smoke.json`, `test -- --suite ui-voice-browser-smoke`                                                                                                                                                      |
...
- `pnpm check -- --scope full --only production-evidence-status` — reports pending non-local production proof requirements from the canonical JSON register.
- `pnpm run check -- --scope release --only production-evidence-complete` — release promotion gate; must fail until every non-local evidence item is reviewed and `verified`.
- `pnpm check -- --scope full --only production-evidence` — verifies the register documentation contract.
- `pnpm test -- --suite core` — passed, 206 files / 1171 tests.
- `pnpm check -- --scope full --only golden` — passed, baseline-check golden.
...
- `pnpm doctor -- --runtime meeting --mission P3-5-REFERENCE-DRIFT-AUDIT` — failed closed as expected because local BlackHole / voice consent are missing; printed bootstrap and consent remediation.
