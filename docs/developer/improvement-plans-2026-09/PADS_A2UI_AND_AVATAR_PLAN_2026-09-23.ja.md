...
PR 1(PA-01〜PA-08)・PR 2(PA-09 / PA-10)とも実装済み。
...
| PA-09         | done | `ui:talking-avatar` + 共通口パクエンジン、音声返却 TTS とブラウザ speech player。相棒 `/work`(ホスト再生は `speech_state` で合成口パク)・`/ask`(ブラウザ再生は音量で口パク、`speechSynthesis` に後退)、秘書室の会話ドックに表示 |
| PA-10         | done | 写真 → 表情セット(参照画像対応プロバイダ + 送信先を示す同意ダイアログ)。生成は `avatar/draft/` に置き、「このアバターを使う」で昇格(それまで今のセットを維持)。配信は所有者限定の `/api/me/avatar`                              |
