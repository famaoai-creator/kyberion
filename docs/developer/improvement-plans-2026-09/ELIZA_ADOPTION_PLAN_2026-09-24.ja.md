...
| FU-02 | プラグインビューの `authority: human` action を、承認後に実行する。承認要求の payload hash と照合し、同じ grant・同じ op preflight を通して 1 回だけ実行、結果を監査に残す                                                                                               | Chronos `plugin-views` route、`plugin-view-contract.ts`、承認の消費側                                     | done |
