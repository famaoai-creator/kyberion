...
- 2026-09-14: **FD-10 仕上げ** — `/work` に残っていた TTS/STT/入力デバイス選択を秘書室 `/settings` の「声と話し方」へ移設。concierge の server-only proxy(`GET` は viewer scope、`POST` は mutation guard)経由で保存し、Presence Studio の Native STT は OS の既定入力を使う。設定画面のメンバー追加フォームをカード / 2 列 / 共通 field label に整え、空白だけの表示名・IDを送信できないようにした。対象 6 ファイル 35 テスト、TypeScript 型検査、ESLint/Prettier を通過。
- **残件(次の PR)**: ワイヤーフレーム PNG の `docs/assets/surfaces/` への保存。Claude Design の合意済み原本がこの checkout に存在しないため、画像を捏造せず、原本の export 後に固定する。
- 2026-09-14: **FD-10 第 1〜2 波(ブランチ `agent/front-desk-fd10-20260914`)** — `libs/core/actor.ts`(`ActorRef` = human / agent / service、`user:<member_id>`、`assertHumanActor`、legacy 文字列の写像)、`AuditEntry.actor`(additive、無い項目のハッシュは不変)、`decideApprovalRequest` / deliverable inbox の判断 seam に `decided_by`、`mission_controller` の `start` / `cancel` / `pause` / `memory-approve` / `memory-reject` に `--decided-by user:<member_id>`(`scripts/lib/decided-by-args.ts`)と秘書室の停滞・記憶ルートからの配線、`personal-agent-identity.schema.json` のフィールド宣言と `agent_id → nhi_id` リンク、`mcp-server-cowork.json` の persona 修正、設定 › 組織とメンバーに責任者付きエージェント一覧、`/work` の音声設定セレクトを「声と話し方」へ移設。**判断**: 成果物受領の書き込みは `security-policy.json` の `surface_runtime` に `active/shared/inbox/` を read / write とも追加して宣言側(`authority-roles/surface_runtime.json` の `communication_surface`)と揃えた(Codex セッションの判断。§5 FD-05 の未決を閉じる)。**修正**: `actor.ts` が `agent-identity.ts` / `member-registry.ts` 経由で `secure-io → audit-chain → actor` の循環 import を作り、全 CLI 入口と監査テストが `__name is not a function` で落ちていたため、NHI id とメンバー id の文法を依存の無い葉モジュール(`nhi-id.ts`、`member-id-grammar.ts`)に分離。`./artifact-verification` / `./training-catalog` の package export 登録。front-desk ブランチ(ルート分割・設定分割)をマージし、ヒアリング / トレーニングのルートは `hearing-routes.ts` / `training-routes.ts`、設定の追加分はセクション部品へ再適用。
- 2026-09-14: **FD-10 追記** — `--decided-by*` を `mission-types.ts` の VALUE_FLAGS に登録(未登録だと `create` の位置引数が崩れる)。相棒サーバが `installReasoningBackends()` を呼んでいなかったため、`/api/conversation` の orchestrator 経路・議事録・ヒアリングキャンバスが常に stub だった問題を修正。**残件(FD-10 原則 2)**: ヒアリングの mission brief 書き込みで surface が `mission_controller` 権限へ昇格している。governed な CLI 受け口を追加して昇格をなくす。
