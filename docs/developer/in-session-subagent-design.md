...
## 5. タスク単位のモデル/エフォート割当方針

`MO-05_MODEL_EFFORT_ROUTING` の運用基準は、ここで定義する。実装時に `claude-sonnet-4` や `claude-haiku` のような文書上の担当モデル名が出てくる場合でも、Codex 経由で作業するときは対応する OpenAI 側モデルへ読み替えてよい。ただし、Anthropic を実際に使うセッションでは表記どおりに扱う。

| タスク特性                           | 既定 tier  | 既定 effort | 運用意図                           |
| ------------------------------------ | ---------- | ----------- | ---------------------------------- |
| mechanical / low risk / narrow scope | `small`    | `low`       | 機械的な横展開を安く速く回す       |
| standard implement / medium scope    | `standard` | `medium`    | ふつうの実装と軽い調整に使う       |
| plan / review / high stakes          | `large`    | `high`      | 設計判断・検証・レビューを厚く取る |

- `KYBERION_TASK_MODEL_ROUTING=advisory` が既定で、ヒントは trace と学習用サマリに残すが挙動は変えない。
- `KYBERION_TASK_MODEL_ROUTING=enforce` は opt-in で、`task_model_hint` がある場合だけ manifest 既定を上書きする。
- `effort` は Anthropic backend では thinking budget に反映し、他 backend では無視してよい。
- 週次レビューでは `pipelines/weekly-review.json` の `summarize_task_model_routing` ステップで `active/shared/tmp/task-model-routing-summary.json` を更新し、モデル選択の rework 率と token 消費を確認する。

## 6. ビジネス的価値（生成AI導入コンサルティングにおける強み）
...
## 7. 会話モデルの節 (A2A Conversation Models)

Kyberion におけるエージェント間（A2A）の通信モデルは、目的や文脈に応じて以下の3つの形態（ステート制御）に分類されます。

### 1. 単発 RPC (Single Request/Response)

- **概要**: 最も単純な1往復の問い合わせ形態。
- **特徴**: `conversation_id` を付与せずに送信されるため、会話ストアには記録されず、履歴の追跡や状態の復元も行われません。現行互換や冪等なクエリに適しています。

### 2. conversation_id 付き多ターン会話 (Multi-turn Conversation)

- **概要**: 連続する複数のコンテキストを持つ会話スレッド形態。
- **特徴**: `conversation_id` 付きで送信され、Turns（リクエスト/レスポンス）が `active/shared/runtime/a2a-conversations/<conversation_id>.jsonl` に記録されます。最大500ターンで自動ローテーションされ、30日間の TTL が適用されます。機密（Confidential/Personal）ミッションのコンテキスト下では、不要なテキスト漏洩を防ぐために履歴保存時にプロンプトや結果のテキストが自動的にマスク・オミットされます。

### 3. 復元会話 (Session Rehydration / Recovery)

- **概要**: クラッシュや再起動により、受信側エージェントのプロバイダセッション（インメモリ状態）が消失した際に、履歴から会話スレッドを復元する形態。
- **特徴**: A2A Bridge がセッション ID の不一致（現在の `sessionId` と会話履歴に記録されている直前の `provider_session_id` の不一致）を検出した場合、自動的に過去の最大10ターン分の会話履歴から復元プロンプト（Rehydration Prefix）を構築し、新規リクエストの先頭に付与します。
- **メタデータ・イベント通知**: 会話が復元されたことは、応答ペイロードのメタデータ (`metadata.rehydrated: true`) および supervisor events の `a2a_conversation_rehydrated` イベントとして明示され、呼び出し側や監査ログが状況を検知できるようになっています。
