...
- `produces.sensitive: true` keeps a channel's value out of the run journal (`active/shared/runtime/pipeline-runs/`): the journal records only `{redacted, sha256, length}` and lists the channel in `redacted_channels`. The run context still carries the value, so later steps and Chronos `deliver_to` templates can use it. A step whose output was redacted is re-executed on resume instead of being restored. Mark any channel that carries confidential or cross-tenant content.
