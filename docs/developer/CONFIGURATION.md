...
| `KYBERION_REASONING_BACKEND`              | string                                       | no       | Reasoning backend selector. Common values: claude-cli (local claude CLI, no API key), anthropic (ANTHROPIC_API_KEY), codex-cli, gemini-cli, agy-cli, openrouter (OPENROUTER_API_KEY or KYBERION_OPENROUTER_KEY), stub (offline/deterministic tests). |
