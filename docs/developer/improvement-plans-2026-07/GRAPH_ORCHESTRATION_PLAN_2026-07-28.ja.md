...
> **位置づけ**: AR-01(ADF エンジン統一)・HN-03(決定論的オーケストレーション原語)・MO-03(タスク DAG 並列分配)・MO-06(durable resume)の**後続ループ**。前提はすべて DONE または PARTIAL([STATUS](./STATUS.ja.md))。[ORCHESTRATION_HARNESS_MODEL](../ORCHESTRATION_HARNESS_MODEL.ja.md) §3/§7 が指摘した「直列 for・バリア同期・workflow-as-code 不在」への構造的回答。
...
- [UX-07_AGENT_COLLABORATION_VISIBILITY](./UX-07_AGENT_COLLABORATION_VISIBILITY.ja.md) — 協調可視化。GE-08 の接続先。
