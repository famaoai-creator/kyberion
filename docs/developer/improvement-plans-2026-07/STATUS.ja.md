...
| DS-04 | PARTIAL | 2026-07-12: agy 縦型ショートの品質修復 — visual direction を LLM 起草 + スキーマ検証で scene CSS トークンへ注入(video-visual-direction.ts)、プレースホルダ工程図の混入全廃、縦型タイポスケール対応。残: motion/transition のトークン化と direction のテーマパック連携 |
