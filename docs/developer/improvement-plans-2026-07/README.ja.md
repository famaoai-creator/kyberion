...
> **履歴台帳**: [STATUS.ja.md](./STATUS.ja.md)(2026-07-05 までの計画をコード実態と突合した凍結記録)。現在の実装状況は [2026-09 の状態索引](../improvement-plans-2026-08/README.ja.md) と各計画文書を参照する。
...
### Kimi Code 概念取り込み(Goal 自律駆動・イベントソーシング復元・信頼と並列実行)

[MoonshotAI/kimi-code](https://github.com/MoonshotAI/kimi-code)(kimi-cli の TypeScript 後継、agent-core-v2 世代)の実コード分析(2026-07-20)に基づく。正本は [KIMI_CODE_ADOPTION_PLAN_2026-07-20.ja.md](./KIMI_CODE_ADOPTION_PLAN_2026-07-20.ja.md)(KD-01〜09 は同文書内)。KC-01〜10 の追補で、TS 版で新登場した概念のみを扱う: ワーカー粒度の Goal 状態機械(active/paused/blocked/complete・構造化終了信号・再監査 continuation)、イベントソーシング型状態復元(MO-06 の実装参照)、由来ベースのプラグイン信頼、リソース宣言型ツール並列化、プロンプトキャッシュ規律。

| ID    | タイトル                                                  | 優先度 | 規模 | 依存                  |
| ----- | --------------------------------------------------------- | ------ | ---- | --------------------- |
| KD-01 | ワーカー Goal 状態機械と自律ドライバ                      | **P0** | M    | KC-02/07/08(実装済み) |
| KD-02 | Goal 予算 grace step・収束モード・deadline                | P1     | S    | KD-01                 |
| KD-03 | イベントソーシング型ワーカー状態復元                      | P1     | M    | KC-02、MO-06連携      |
| KD-04 | untrusted 入力の注入枠付け契約                            | P1     | S    | なし                  |
| KD-05 | サブエージェント能力ティア(型付き allowlist プロファイル) | P1     | S    | KC-06(実装済み)       |
