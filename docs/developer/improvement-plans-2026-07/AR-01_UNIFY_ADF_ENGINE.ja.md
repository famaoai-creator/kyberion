...
- **run_pipeline 委譲 Phase B 完了(2026-07-13)**: 下記設計メモの Phase B を実施。`runSteps` の `while (attempt < 2 …)` + catch 内 on_error/autonomous-repair ロジックを、ステップの1回分の実行(`attemptOnce`)を引数に取る高階関数 `runStepWithRepair` に外出しした。`attemptOnce` は control-op の分岐(if/while/foreach/parallel_foreach/accumulate/include/reasoning/dispatch)を verbatim 保持したまま、結果を `{kind:'done'|'skip'}` か(`core:include` のネスト失敗のみ)`{kind:'early_return', result}` で返す判別共用体にした — 元のコードが `continue`(スキップ)や `return`(即時失敗復帰)を while ループ内に直接書いていたのを、呼び出し元が解釈できる信号に変換しただけで意味論は不変。on_error 処理も `tryOnErrorRecovery` として同様に外出し。**目的**: canonical エンジン(`executeAdfSteps`)は retry を持たない設計のため、Phase C で委譲する際にリトライ/修復をエンジン側でなく「ハンドラをラップする関心事」として持ち込める形にする準備。純粋な構造リファクタで、分岐条件・ログ文言・repair 呼び出し順序は変更していない。検証: `npx tsc --noEmit` 緑、run_pipeline 36テスト緑、`pnpm run build` 後 `pnpm run check:golden` 緑(2 パイプライン)。**残: Phase C(`executeAdfSteps` への実委譲)のみ**。

- **Phase C 完了(2026-07-14)**: `run_pipeline.ts` の私製ループ(if/while/foreach/parallel_foreach/accumulate/include の制御分岐、~750行)を全廃し、`executeAdfSteps(steps, ctx, {maxSteps, timeoutMs, resolveVars}, handlers, hooks)` への委譲に置換。**受入条件1が完成**(正準エンジンが唯一のエンジンに)。
  - **ルーティングは type/role でなく正規化 op で判定**: `resolveStepType`(AR-03/AR-04 由来のアクチュエータ dispatch ヒント)は role/type 未指定の `core:if` 等に対して実質 `'apply'` を返す(登録レジストリに if/while 等が無く例外→デフォルトへフォールバックするため)。これはエンジンへの経路選択には使えないと判明したため、`resolveEngineStepType` を新設 — 正規化 op が6つの制御アクション(if/while/loop_until/retry_until_quality/foreach/parallel_foreach/accumulate/include)に一致する場合のみ `'control'`、それ以外は一律 `'apply'` に固定(capture/transform ハンドラは interface 充足のためのダミーで、実行されない)。`resolveStepType` 自体は変更せず、アクチュエータ dispatch ヒント/capture 安全確認という元の役割のまま併存。
  - **on_error はエンジン native 経路に一本化**: `run_pipeline` 独自の on_error ハンドリング(`tryOnErrorRecovery`)を廃止し、`handleStepError` を直接使うエンジンの既存 on_error 経路(file/network/等の8アクチュエータと共通)へ寄せた。理由: リトライ/repair はハンドラ内部(エンジンから不可視)で完結させる設計のため、on_error 付きステップで「ハンドラ内 on_error 失敗→エンジンが再度同じ on_error を発火」という fallback の二重実行を避ける必要がある。解: on_error が宣言されたステップは repair を一切試みず素通しでエンジンへ例外伝播し、on_error 無しのステップのみ `runWithRepair`(Phase B の高階関数、on_error 分岐を除いたもの)でラップ。副作用: `status:'recovered'` は常にエンジン由来になり、`run_pipeline` 側での二重管理が消えた。
  - **flatten は hooks 発火順から再構成**: `beforeStep`/`afterStep` はネストしたステップにも発火する(adf-engine.test.ts で既存確認済み)ため、最終 `results` 配列を `afterStep` の発火順にそのまま追記するだけで、旧コードが手動で `results.push(...nested.results)` していた「フラット化」が副作用として得られる。`opts.trace` の span/event・quiet ログ・`step.hooks.before/after`(StepHook)・on_error 無しステップの repair も全て `beforeStep`/`afterStep`/ハンドラ内に移設。
  - **エンジン側の最小加算1点**: `AdfStepHandlers.apply` が `AdfSkippedStep` を返せるよう拡張(`capture`/`transform`/`control` は変更なし)。理由: `run_pipeline` の非制御 op は全て `'apply'` にルーティングされるため、`step.hooks.before` の skip 判定をこのハンドラ経由で表現する必要があった。既存 handler(plain ctx を返すのみ)は無変更で動作 — 加算的な変更。
  - **意図的な意味論変更(文書化・許容範囲と判断)**:
  ...
  - **検証**: `scripts/run_pipeline.test.ts` 38本(既存36本 + core:include 正常系/循環検出の新設2本、全て正準エンジン経由の in-process 実行)+ `libs/core/adf-engine.test.ts` 12本(既存10本 + apply-skip の新設1本)全緑。`pnpm run build`(全ワークスペース)・`pnpm run build:actuators` 緑。`pnpm run check:golden`(2パイプライン)緑。file/network/code/modeling/system/wisdom/browser/media 全アクチュエータ 301本緑(共有エンジンの後方互換性を確認)。`node dist/scripts/run_pipeline.js --input pipelines/vital-check.json` と `--input pipelines/reconcile-config-fallbacks.json`(core:if 分岐含む)を実 CLI で実行し回帰なしを確認(ログにネスト flatten が正しく現れることも目視確認: `[step 5/6] system:log success` → `[step 5/6] core:if success`)。
...
2. **Phase B(retry/repair のステップ内在化)— 2026-07-13 完了**: `attempt < 2` + autonomous-repair を「ステップハンドラをラップする高階関数」(`runStepWithRepair`)に変換(canonical エンジンは retry を持たない設計のため、リトライはハンドラ内部関心事に落とす)。on_error 処理も同様に `tryOnErrorRecovery` として外出し。run_pipeline 36テスト + golden 2本緑で回帰なし。
