...
| ID    | 状態 | 備考                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ----- | ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| WI-18 | DONE | `requireConciergeSelfServiceAccess`(CSRF・レート制限のみ、役割判定なし)を同意の付与・撤回と要約の確認・破棄にだけ使う。本人判定はサーバー側の有効メンバー解決。業務への取り込みは従来どおり管理権限                                                                                                                                                                                                                                     |
| WI-19 | DONE | 原因は CodeQL の `IndirectCommandArgument`: 呼び出し元の一部が汎用ヘルパー経由で `sh`/`bash`/`cmd` + `-c`/`/c` を起動していたため、ヘルパーの引数全体がシェル扱いになっていた(呼び出し元を区別しない判定)。意図的なシェル実行を `safeExecShellScript` / `safeExecShellScriptResult` に分け、汎用ヘルパーはその形を拒否する。CodeQL CLI 2.27.0 の手元解析で 698 → 8 件(残り 8 件は `execSync` に文字列コマンドを直接渡す既存 2 ファイル) |

移行の注意: 手書きのパイプラインで `system:exec` から `bash -c` を呼んでいる場合は `[SECURITY]` エラーになる。`system:shell` に切り替える(CHANGELOG に記載)。
