...
| SX-03 | PARTIAL     | `foundation/{ajv,json,env,text,time}` を追加し、旧JSON読み込み・env読み取り・JSONL appendをfoundation経由へ移行。`env-validator` の互換APIも foundation parser へ委譲し、foundation adoption checkはJSON/Ajv/env/JSONLを全て0で固定した。意味の異なる私的text/clamp helperの統合、secure-ioの残存低レベル経路整理は未完。                                                                                                                            |
...
| SX-08 | PARTIAL     | voice の旧梯子を削除し共有 `generateReply` を入口化。voice-hub の `replyText` と `IntentResolutionContract` を Concierge へ伝播し、clarification/approval の shape と next action を描画可能にした。intent CLI統合、Chronos/bridgeを含む全surface描画、approval本番経路接続は未完。                                                                                                                                                                  |
| SX-09 | PARTIAL     | `ChannelAdapter`/`runChannelTurn` を追加し、4 bridgeの共通turn lifecycleとthreadContextを採用。provider-neutralなthread formatterと、text-only channelへapproval/clarificationのnext action・consequenceを配送する共通formatterを追加した。viewer resolver 3系統、bridge固有thread履歴、read-model・配送統合は未完。                                                                                                                                 |
