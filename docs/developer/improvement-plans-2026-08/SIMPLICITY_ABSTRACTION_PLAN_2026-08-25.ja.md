...
> | SX-11 | PARTIAL | `runAdfLifecycle` と canonical repair を実行入口へ接続し、`core:include` に fragment context/result envelope を追加。raw command形式に加えて`command + args`形式の`node dist`/`pnpm exec`/`npx tsx` wrapperも拒否するguardrailを追加した。実pipeline監査は75件・56ファイルを検出し、既存pipelineの置換とsuper-nerveの重複repair・語彙移行は未完。 |
