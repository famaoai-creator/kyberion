...
> **2026-08-25 実測更新**: 以下の初期値は監査時点のスナップショットとして保持する。実装波後の検証値は `JSON.parse(safeReadFile(` が **161 occurrences (134 non-test files)**、自前 `new Ajv(` が **99 occurrences (98 non-test files)**、`process.env.KYBERION_*` の non-test が **400 occurrences (201 non-test files)**、`defineCatalog` が **24 core files** となった。循環を増やすコア直読移行はfoundation環境層へ寄せ、目標未達の項目は引き続き PARTIAL とする。
