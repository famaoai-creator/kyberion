...
> **2026-08-25 実測更新**: 以下の初期値は監査時点のスナップショットとして保持する。実装波後の検証値は、空白を含まない厳密な `JSON.parse(safeReadFile(` と空白許容パターンがともに **0 occurrences**、foundation 外の自前 `new Ajv(` が **0 occurrences**、`process.env.KYBERION_*` の non-test が **0 occurrences**、旧式JSONL append が **0 occurrences**、`defineCatalog` が **65 core implementations**、import boundary は **177 cycles / 114 direction violations** となった。JSON/env/Ajv/JSONL の foundation adoption とCLI未知コマンドのfail-closed化は完了した。さらに、参照のないpackage script 19件（build内参照のある `build:tui` は除外）を削除し、受入check alias 3件を保持してnpm scriptsは **278→262** となった。check名の受入契約はpackage script連鎖ではなく`ci-gates.json`のmanifestで検証するようテストを更新した。私的 text helperの意味統合、全catalogのschema根統合、cycle削減、CLI/harness全移行は未完のため、SX-03〜07は引き続き PARTIAL とする。
...
| npm scripts                                              | 278                                                         | ≤120   | SX-05    |
| CLI 入口                                                 | 5                                                           | 1      | SX-05    |
| doctor/preflight 実装                                    | 11                                                          | 1      | SX-05    |
| `process.argv` 直読 script                               | 0（harness境界を除くproduction scripts）                    | 0      | SX-06    |
| `check:*` で CI 未実行                                   | 32                                                          | 0      | SX-07    |
| PR 前に打つコマンド                                      | 6                                                           | 1      | SX-07    |
| 自由文解釈の実装                                         | 6                                                           | 1      | SX-08    |
| `IntentResolutionContract` 描画 surface                  | 共通formatter + 4 bridge + Concierge/Chronos/Terminal/voice | 12/12  | SX-08    |
| viewer/auth 実装                                         | 5                                                           | 1      | SX-09    |
| vocabulary lookup 実装                                   | 6                                                           | 1      | SX-09    |
| actuator ABI                                             | 4                                                           | 1      | SX-10    |
| op 入力 schema カバレッジ                                | 13%                                                         | 100%   | SX-10    |
| `system:exec` スクリプト包み                             | 75                                                          | 0      | SX-11    |
| 最大ファイル行数                                         | 6,041                                                       | ≤1,500 | SX-12    |
...
| SX-03 | PARTIAL     | `foundation/{ajv,json,env,text,time}` を追加し、旧JSON読み込み（本番コードの `JSON.parse(String(safeReadFile(...)))` は0件）・env読み取り・JSONL appendをfoundation経由へ移行。`env-validator` の互換APIも foundation parser へ委譲し、provider-health/operator-identity/locale/format/generation-quota/ingest-quota/browser-onboarding、system focus、history index、work graph、retention catalog、lifecycle hooks、plugin pack、curation/feedback、performance index、visual raster、PII rules、intent track、mission event、audio lease、email workflow、AIDLC state、artifact review、storage janitor、mission governance/journal/reconciliation、scope offboarding、tenant activation、writer lease、補助scriptおよびsurface setupのruntime JSON読み込みも `readJson` へ寄せた。今回、Ajvの外部ref登録をfoundationへ統合し、旧`schema-loader`はcaller-owned Ajv用の互換境界へ縮小、6 registryをfoundation APIへ移行し、ADF修復・背景レビューの知識抜粋も共通text helperへ統合した。Ajv/env/text/timeの残存codemod、意味の異なる私的clamp helperの統合、secure-ioの残存低レベル経路整理は未完。                                                                                                                                                                                                                                                                                                                                    |
| SX-04 | PARTIAL     | `defineCatalog<T>()` を追加し、対象policy loaderをschema付きへ移行。CLI registry、mission review gate、service bootstrap catalog、approval policy、reasoning level/backend policy、voice task profile catalog、voice engine registry、service runtime policy、voice runtime policy、shell command policy、egress policy、service onboarding、surface query overlay、work coordination import、process definition、service authority、actuator operation catalog、AWS icon rule、tone style mapも共通ローダーへ移行し、egress schemaのmode/link語彙も実装契約へ整合させた（core内のdefineCatalog利用は55実装ファイル）。105件のroot schemaを`knowledge/product/schemas/`へ移し、5件の同名別契約は意味名へ分離し、実コードのschema参照もcanonical rootへ統合した。governance rootにはschemaまたはdocumentation_only分類のないJSONが157件残り、全loader移行・未参照catalog処分は未完。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| SX-05 | PARTIAL     | command registryを`id/noun/verb/entry/audience`単位へ拡張し、schema検証・entrypoint route整合性・未知commandのfail-closedを追加。doctorを`kyberion doctor`の実導線へ統合し、`kyberion --help`でregistry由来のユーザーコマンド一覧を表示するようにした。ci-gatesの直接実行化に加え、依存 pinning・install-script allowlist・lockfile・golden をmanifest gateへ移し、CI/releaseの個別alias呼び出しを`pnpm check`へ統合した。npm scriptsは326→278→262→258→247→243へ削減し、参照のない開発・bridge alias 11件を処分した。残りのalias整理と120以下への削減は未完。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| SX-06 | PARTIAL     | `defineScript`/`defineGenerator` と harness test を追加し、script ごとの `flags` 宣言を可能にした。pseudo-locale / vocabulary-types / env-registry / op-registry / model-registry / intent-contract-memory / deterministic checkers / pii-rules / trace-docs / capability-seams / subagent-definitions / doctor / CLI manifest checker に加え、UX contract、improvement-plan metadata、module boundaries、UI/UX governance、wire error boundary、knowledge scope、production evidence、governance rules、deprecated wisdom、i18n hardcoding、Chronos contrast、tier hygiene、tenant registry、type ratchet、entity governance、event wiring、mission-gate docs、design ledger、channel adapter checker、mission-process-bindings、pipeline-shell-independence、foundation-adoption、contract-semver、backend-conformance、doc-examples、Chronos perf、catalog integrity、first-win smoke、op-input contract coverage、golden output、check orchestrator、license audit、software quality report、dependency vulnerability scan、baseline check、utility/customer/peer/evaluation/company/provider CLIsを共通 harness へ移行。生成器の出力宣言・比較正規化・実行コンテキストも共通化し、状態CLIの戻り値・entrypoint guard・preflight/audit statusを共通化した。production scripts の `process.argv` 直読と直接 `process.exit()` は、harness自身とテスト/移行補助コードを除き0件へ削減したが、孤児実行体・generator移行の全件完了は未完。 |

> **最新メトリクス (2026-08-25)**: SX-06のproduction scriptsにおける`process.argv`直読は0ファイル、直接`process.exit()`は0ファイル（harness本体・test/refactor補助コードは境界/対象外として除外）。`serve-brief`、`report-review/server`、`onboarding_wizard`、`org`を`main(args)` + harnessへ移し、temp workspaceからの相対TS importもloaderのproject-root解決で維持した。残存する40件の終了処理も`process.exitCode`/既存のcaller error boundaryへ統一し、actuator scaffoldは`import.meta.main`でentrypointを判定する。孤児実行体・generator移行の全件完了は未完。
> **SX-05/SX-07 実装追記 (2026-08-25)**: command registryに登録済みだった`doctor`を`kyberion_home`のsingle CLI switchから共有`runDoctor`へ接続し、`kyberion doctor --runtime baseline`の実導線を修復した。`--json`も共有reportを構造化出力し、`kyberion --help`はregistry全commandを列挙する。ci-gatesの直接実行形式への移行に加え、依存 pinning・install-script allowlist・lockfile・golden をmanifestへ追加し、CI/releaseの個別checker aliasを`pnpm check`へ統合した。PR scopeは25 gate、fullはPR継承を含む62 gateとなり、直近の局所検証はgreen。doctor aliasと残存npm scriptsの整理は未完（243件まで削減）。
> | SX-07 | PARTIAL | manifest-driven `pnpm check` とPR workflow gateを追加し、未知scope・空集合・scope外`--only`・未知flagをfail-closed化。post-buildのop-preflight、extension/module invariant、seam、capability、reasoning provider、packaging、ESM、knowledge scope、design ledger、tenant registry、documentation source mapをmanifestへ統合し、CI/PR workflowの個別checker重複を削除した。PRは25 gate、fullはPR継承を含む62 gateを同一orchestratorで実行し、最新実行で全てgreenだった。PR title・push時commit subject・lint/format/test/security・artifact/runtime smokeはイベント/環境固有の例外としてworkflowに残るため、完全な個別checkerゼロではなくPARTIALを維持。 |
> | SX-08 | PARTIAL | voice の旧梯子を削除し共有 `generateReply` を入口化。voice-hub の `replyText` と `IntentResolutionContract` を Concierge へ伝播し、clarification/approval の shape と next action を描画可能にした。Chronos `/api/agent` も契約を構造化 JSON と A2UI section へ出し、承認UIは `action: approve_mission` を送るようにした。`kyberion ask --explain` も同じ共有入口の契約を表示する。今回、text-only bridge の共通formatterを4項目（理解・不足・次の行動・成果）へ拡張し、Chronos A2UI、Terminal HUD、voice-hubの音声/テキスト出力にも同じ契約を反映した。`run_intent.ts`のcompile/delegate gateway、`scripts/intent.ts`のtrace検査、残りsurfaceの本番描画監査、approval本番経路接続は未完。 |
...
> | SX-11 | PARTIAL | `runAdfLifecycle` と canonical repair を実行入口へ接続し、`core:include` に fragment context/result envelope を追加。raw command形式に加えて`command + args`形式の`node dist`/`pnpm exec`/`npx tsx` wrapperも拒否するguardrailを追加した。実pipeline監査は75件・56ファイルを検出し、既存違反をbaselineとして新規regressionを遮断するところまで実装した。今回、native actuator opへ3件を移行し、残存baselineは72件・53ファイル。super-nerveの重複repair・語彙移行と残存wrapperの移行は未完。 |
> | SX-12 | PARTIAL | 21 seam の生成入口を `createSeam<T>()` に統一し、`libs/core/index.ts` を11個の境界付きpartへ分割、planning packet/review helpersを `mission-planning-packet.ts` へ抽出した。今回、phase gate評価を `mission-orchestration-phase-gates.ts`、NEXT_TASKS/TASK_BOARD同期を `mission-orchestration-progress.ts`、task validation/artifact review/planning/task response/lifecycle handlers/graph dispatchを専用境界へ移し、workerのpayload/task契約も `mission-orchestration-worker-contracts.ts` へ抽出した。最大god moduleは6,036行から3,901行まで減少し、worker回帰29/29と型検査を通過。store/façade・状態機械の統合と1,500行目標は未完。 |
> | SX-13 | PARTIAL | 状態正本を2026-08 READMEへ一本化し、2026-07 STATUSを履歴台帳へ降格。完了statusの7計画本文を月別`improvement-plans-archive/`へ移動し、既存参照を更新した。GlossaryはFirst-win / Contributor / FDEの3層、不足5語を明記。`_integrity-manifest.json` と frontmatter exclusion manifestを追加し、検索は`knowledge-slices`経由に明記、テスト生成のincident 19件を削除した。distill文書は証跡を含むため保持。 |
...
> **SX-06/SX-12 実装追記 (2026-08-25)**: production scriptsの直接`process.argv`/`process.exit()`を0件へ固定するscript-integrity ratchetを追加し、workerのpayload/task契約を`mission-orchestration-worker-contracts.ts`へ抽出した。さらにphase gate/progress、task validation、artifact review、planning、task response、lifecycle handlers、graph dispatch境界をworkerから分離し、worker本体は3,901行まで減少したが、SX-12の1,500行目標および残存god module分割は未完。
> **SX-12 残差追記 (2026-08-25)**: `scripts/run_pipeline.ts` の失敗分類・次アクション整形を `scripts/pipeline-result-reporting.ts` へ分離した。CLI互換のre-exportを維持し、run_pipeline本体は3,564行、既存run_pipeline回帰55/55とtypecheckを通過。god module全体の1,500行目標は未完。
