...
| SX-06 | PARTIAL     | `defineScript`/`defineGenerator` と harness test を追加し、script ごとの `flags` 宣言を可能にした。pseudo-locale / vocabulary-types / env-registry / op-registry / model-registry / intent-contract-memory / deterministic checkers / pii-rules / trace-docs を共通 harness へ移行。既存 script 全移行は未完。 |
