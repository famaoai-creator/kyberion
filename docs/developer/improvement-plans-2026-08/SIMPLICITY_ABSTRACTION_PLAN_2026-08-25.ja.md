...
| SX-07 | PARTIAL     | manifest-driven `pnpm check` とPR workflow gateを追加し、未知scope・空集合・scope外`--only`・未知flagをfail-closed化。post-buildのop-preflight、extension/module invariant、seam、capability、reasoning provider、packaging、ESM、knowledge scope、design ledger、tenant registryをmanifestへ統合し、CI/PR workflowの個別checker重複を削除した。PRは20 gate、fullはPR継承を含む57 gateを同一orchestratorで実行する。PR title・push時commit subject・lint/format/test/security・artifact/runtime smokeはイベント/環境固有の例外としてworkflowに残るため、完全な個別checkerゼロではなくPARTIALを維持。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
