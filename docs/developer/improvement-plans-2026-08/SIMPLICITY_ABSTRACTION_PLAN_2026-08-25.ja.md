...
| SX-04 | PARTIAL     | `defineCatalog<T>()` を追加し、対象policy loaderをschema付きへ移行。CLI registry、mission review gate、service bootstrap catalog、approval policy、reasoning level/backend policy、voice task profile catalog、service runtime policy、voice runtime policy、shell command policyも共通ローダーへ移行し、customer overlayの既存入力形式をschemaへ反映した（core内のdefineCatalog利用は42ファイル）。governance rootにはschemaまたはdocumentation_only分類のないJSONが157件残り、全loader移行・schema根統合・未参照catalog処分は未完。                                                                                                                                                                                                                                                                                                                                                                  |
