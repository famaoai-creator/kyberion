...
| SX-04 | PARTIAL     | `defineCatalog<T>()` を追加し、対象policy loaderをschema付きへ移行。CLI registry、mission review gate、service bootstrap catalog、approval policy、reasoning level/backend policy、voice task profile catalog、service runtime policy、voice runtime policy、shell command policy、egress policy、service onboarding、surface query overlay、work coordination import catalogも共通ローダーへ移行し、egress schemaのmode/link語彙も実装契約へ整合させた（core内のdefineCatalog利用は46ファイル）。governance rootにはschemaまたはdocumentation_only分類のないJSONが157件残り、全loader移行・schema根統合・未参照catalog処分は未完。                                                                                                                                                                                                                                                                    |
