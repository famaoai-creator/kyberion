...
> **SX-05 実装追記 (2026-08-25)**: command registryに登録済みだった`doctor`を`kyberion_home`のsingle CLI switchから共有`runDoctor`へ接続し、`kyberion doctor --runtime baseline`の実導線を修復した。`--json`も共有reportを構造化出力する。doctor aliasの整理とnpm scripts削減は未完。
