...
> **最新メトリクス (2026-08-25)**: SX-06の`process.argv`直読は25ファイル、直接`process.exit()`は59ファイル。`run_pipeline.ts`の終了経路を`process.exitCode`とcaller error boundaryへ統一し、状態を持たないentrypoint guardも`isDirectScript`へ統一、preflight/audit statusをharnessへ寄せた。状態を持つcommand CLIの全移行は未完。
