...
| `process.argv` 直読 script                               | 28    | 0      | SX-06    |
...
> **最新メトリクス (2026-08-25)**: SX-06の`process.argv`直読は28ファイル、直接`process.exit()`は61ファイル。状態を持たないentrypoint guardも`isDirectScript`へ統一し、preflight/audit statusをharnessへ寄せた。状態を持つcommand CLIの全移行は未完。
