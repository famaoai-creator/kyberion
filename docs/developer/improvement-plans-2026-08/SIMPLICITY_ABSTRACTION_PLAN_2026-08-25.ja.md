...
> | SX-11 | PARTIAL | `runAdfLifecycle` と canonical repair を実行入口へ接続し、`core:include` に fragment context/result envelope を追加。raw command形式に加えて`command + args`形式の`node dist`/`pnpm exec`/`npx tsx` wrapperも拒否するguardrailを追加した。実pipeline監査は75件・56ファイルを検出し、既存違反をbaselineとして新規regressionを遮断するところまで実装した。既存75件のnative op/core:include置換とsuper-nerveの重複repair・語彙移行は未完。 |
> | SX-12 | PARTIAL | 21 seam の生成入口を `createSeam<T>()` に統一し、`libs/core/index.ts` を11個の境界付きpartへ分割、planning packet/review helpersを `mission-planning-packet.ts` へ抽出した。今回、phase gate評価を `mission-orchestration-phase-gates.ts`、NEXT_TASKS/TASK_BOARD同期を `mission-orchestration-progress.ts` へ移し、workerのpayload/task契約も `mission-orchestration-worker-contracts.ts` へ抽出した。最大god moduleは6,036行まで減少し、worker回帰29/29と型検査を通過。store/façade・状態機械の統合と1,500行目標は未完。 |
