...
> | SX-07 | PARTIAL | manifest-driven `pnpm check` とPR workflow gateを追加し、未知scope・空集合・scope外`--only`・未知flagをfail-closed化。post-buildのop-preflight、extension/module invariant、seam、capability、reasoning provider、packaging、ESM、knowledge scope、design ledger、tenant registry、documentation source mapをmanifestへ統合し、CI/PR workflowの個別checker重複を削除した。PRは21 gate、fullはPR継承を含む58 gateを同一orchestratorで実行し、最新実行で全てgreenだった。PR title・push時commit subject・lint/format/test/security・artifact/runtime smokeはイベント/環境固有の例外としてworkflowに残るため、完全な個別checkerゼロではなくPARTIALを維持。 |
...
> **SX-06/SX-12 実装追記 (2026-08-25)**: production scriptsの直接`process.argv`/`process.exit()`を0件へ固定するscript-integrity ratchetを追加し、workerのpayload/task契約を`mission-orchestration-worker-contracts.ts`へ抽出した。さらにphase gate/progress境界をworkerから分離し、worker本体は6,041行まで減少したが、SX-12の1,500行目標および残存god module分割は未完。
