...
| SX-08 | PARTIAL     | voice の旧梯子を削除し共有 `generateReply` を入口化。`IntentResolutionContract.next_action` を追加し、approval/clarification/continueの帰結を型・schema化した。intent CLI統合、全surface描画、approval本番経路接続は未完。                                                                                                                                                                                                                           |
