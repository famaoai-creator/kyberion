...
| `process.argv` 直読 script                               | 25                                                          | 0      | SX-06    |
...
> | SX-12 | PARTIAL | 21 seam の生成入口を `createSeam<T>()` に統一し、`libs/core/index.ts` を11個の境界付きpartへ分割、planning packet/review helpersを `mission-planning-packet.ts` へ抽出した。最大god moduleは6,740行で、store/façade・状態機械の統合は未完。 |
