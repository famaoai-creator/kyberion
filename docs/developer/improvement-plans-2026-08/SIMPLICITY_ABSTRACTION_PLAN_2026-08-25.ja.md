...
| SX-03 | PARTIAL     | `foundation/{ajv,json,env,text,time}` を追加し、旧JSON読み込み・env読み取り・JSONL appendをfoundation経由へ移行。`env-validator` の互換APIも foundation parser へ委譲し、provider-health/operator-identity/locale/format/generation-quota/ingest-quota/browser-onboarding、system focus、history index、work graph、retention catalog、lifecycle hooks、plugin pack、curation/feedback、performance index、visual raster、PII rules、intent track、mission event、audio lease、email workflow、AIDLC state、artifact review、storage janitor、mission governance/journal/reconciliation、scope offboarding、tenant activation、writer leaseおよび補助scriptのruntime JSON読み込みも `readJson` へ寄せた。foundation adoption checkはJSON/Ajv/env/JSONLを全て0で固定したが、専用状態パーサー等の同形JSON読込が本番コード13ファイル残り、意味の異なる私的text/clamp helperの統合とsecure-ioの残存低レベル経路整理も未完。 |
