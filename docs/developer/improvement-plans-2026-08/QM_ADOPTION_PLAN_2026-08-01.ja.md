...
- **QM-06: コア完了(バッチ⑤)。** `backend-capability-profile.ts` — 全 20 モードの transport/能力/utility 適性を宣言(Record 型で網羅性をコンパイル時強制。stub は utility_fit 空 = wisdom 系が絶対に route しない宣言)。`ReasoningBackend.resetSession?` フックを追加し、**failover の serving candidate が切り替わった呼び出しで新旧両 backend の provider セッションをリセット**(qm harness-router パターン)。**追加実装**: `reasoning-route-resolver` が built-in mode の capability profile を参照し、route provenance に backend profile を記録、profile が宣言しない vision 等を要求時に reject。**残**: 各 CLI を実際に叩く適合マトリクス(宣言の実証)。
...
- **QM-06 残の解消(バッチ⑥)**: `resetSession` を実セッション保持 backend に実装 — copilot(ACP mediator shutdown + boot 再武装)、codex-cli(共有 app-server session の drop + best-effort shutdown)。wrapper(`RoleAwareReasoningBackend` / `DispatchingReasoningBackend`)は内包 backend へ転送。残: CLI 実行の適合マトリクス(宣言の実証)のみ。
