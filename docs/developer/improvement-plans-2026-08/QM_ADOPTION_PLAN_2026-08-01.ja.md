...
- **QM-02: 先行記録。** レジストリ可搬性: `normalizeScheduledPipelinePath`(root 内絶対→相対、レガシー絶対は `pipelines/` セグメントから移行、移行不能は拒否)+ `resolveScheduledPipelinePath`、chronos_daemon が解決を使用、実レジストリ 14 件を相対化。トリガ経路・権限・プロセス watch の完了状態は下記のミッション記録を正本とする。
- **QM-03: 完了(バッチ③、ミッション `QM-ADOPTION-MEMORY-B3`)。** `libs/core/memory-notebook.ts` 新設(行文法の単一正本、`foldCapture` = provenance 中立化 + 正規化 dedupe + 日付付与 + MAX_FACTS 最古落ち、`queryBullets`、consolidation アクション文法 UPDATE/DELETE/ADD/NONE + watermark マーカー + `planConsolidation`)。working-memory actuator の `opNote` が fold 準拠に(日付・dedupe・untrusted provenance 書換。セクション構造ファイルのため cap は意図的に非適用)。promotion queue / distill promotion の fold 正本組込、`scripts/benchmark_memory.ts` と `bench:memory` 契約に加え、`background-review-patch` の memory consolidation候補→承認→hash-bound apply→backup を配線。
...
- **QM-08: 完了(バッチ④、ミッション `QM-ADOPTION-OPS-B4`)。** `PACKAGING_CONTRACT.md` に配布契約の条項ステータス表(ENFORCED / VALIDATED-ONLY / RESERVED + 検証器名)を新設。`scripts/check_packaging_contract.ts` が ENFORCED 条項(image.tier-isolation の .dockerignore 除外群、config.no-secret-values の env.example 値検出)を機械検証し、package.json + CI 両ワークフローに登録。onboarding wizard/apply は profile 配下へ no-secret-values の runbook skill と provenance sidecar を生成する。
- **QM-10: 完了(バッチ④)。** `tests/docs-honesty-contract.test.ts` 新設 — AGENTS.md の不変条件(secure-io lint 境界・symlink 正本・plugin fail-closed・tier 定義)、**条項表の ENFORCED 行が実在する検証器を指すこと**、posture 単調性、採択計画 §6 の主要な主張をコードと突合。qm 思想 7 原則を `kyberion-development-practices.md` §7 に distill(fail-open ラベル・純関数ポリシー・切詰め=審査不能・rename 禁止・正直な限界宣言・非対称脱難読化・単調強化)。**追加実装**: `libs/core/store-contract.ts` の共通JSON record store契約を approval store / surface outbox の代表テストへ適用。
- **QM-09: コア完了(バッチ④)。** `libs/core/gap-phase.ts`(語彙 = コード単一正本、recorder、書込時語彙検証 `sanitizeGapSamples`)。`DelegatedTaskTrace.gap_phases` を追加し、`delegateTaskWithUntrustedData`(`onGapPhases` オプション)と adf-repair-agent の trace 完了に配線。**追加実装**: mission dispatch の context_pack / backend_dispatch を mission trace に記録し、Chronos TraceViewer に内訳表示を追加。語彙の governance JSON registry 化は「コードが registry」方式に変更(checker は docs-honesty テストが兼務)。
- **QM-11: 完了(バッチ⑤、ミッション `QM-ADOPTION-BACKEND-B5`)。** `egress-policy.ts` に `composeAudienceFloor`(allow **∩** / deny **∪**、ホスト正規化、サブドメイン継承、空 audience = 全拒否)+ `evaluateAudienceEgress`。`sendToCustomer` は本文中リンクを tenant+operator の合成床で検査し、違反があれば承認ゲート前に拒否(1 参加者の拒否が全員に効く)。新規 ID 基盤は作らず既存 tenant/operator ポリシー保持者で合成。
- **QM-06: 完了(バッチ⑤)。** `backend-capability-profile.ts` — 全 20 モードの transport/能力/utility 適性を宣言(Record 型で網羅性をコンパイル時強制。stub は utility_fit 空 = wisdom 系が絶対に route しない宣言)。`ReasoningBackend.resetSession?` フックを追加し、failover時に新旧両 backend の provider セッションをリセット。`reasoning-route-resolver` は capability profile を参照し、route provenance に backend profile を記録、profile が宣言しない vision 等を要求時に reject。`backend-conformance.ts` / `check:backend-conformance` が各CLIの version/help を実際に叩く適合マトリクスを生成し、構造化出力等は `verified/declared/unavailable` を区別する。2026-08-08の証跡は `evidence/QM-06-CLI-MATRIX-20260808.json`。copilot CLI は未導入のため unavailable と正直に記録。
- **QM-07: 完了(バッチ⑥、ミッション `QM-ADOPTION-PACKS-B6`)。** `libs/core/plugin-pack.ts` — git plugin pack の取込ライフサイクル: https 限定 URL ガード(userinfo / IP / localhost 拒否。DNS rebinding は honest scope 外と明記)、`pinned/tracked`、pack registry(`active/shared/plugins/packs.json`)+ 全試行の ImportRecord(JSONL + 監査チェーン)、**他 pack / 手動インストール済み plugin id の占有は skip**(上書きしない)、上流削除は**アーカイブ**(削除しない)、fetch を跨ぐ fingerprint ガード(「pack changed while fetching」)。個々の plugin は既存 `installPluginManaged` を通るため **third-party = pending_approval で、承認まで絶対に実行されない**(KD-06 契約をそのまま継承)。CLI: `pnpm plugin:install --pack <https-git-url> [--ref] [--tracked]`。
- **QM-06 残の解消(バッチ⑥)**: `resetSession` を実セッション保持 backend に実装 — copilot(ACP mediator shutdown + boot 再武装)、codex-cli(共有 app-server session の drop + best-effort shutdown)。wrapper(`RoleAwareReasoningBackend` / `DispatchingReasoningBackend`)は内包 backend へ転送。CLI version/help matrix は `QM-06-CLI-MATRIX-20260808.json` に記録し、未導入CLIとモデルターン未実行の項目は宣言と分離している。
- **QM-02: 完了(ミッション `MSN-QM02-TRIGGER-20260808`)**。`libs/core/trigger-runner.ts` を新設し、cron/watch/wake を同一 idempotency + delivery receipt 経路へ統一。store lock による原子 claim、期限付き pending、同一 delivery ID の失敗再試行、canonical authority role/level と active execution role の突合、監査障害の非破壊化、store compaction を実装。Chronos cron は leader lease と `chronos_gateway` execution context、fresh run ID/runtime context を使用し、`managed-process.ts` の `armWatch`/`armTriggerWatch` は quiet 単発化、tail byte 上限、listener cleanup、callback rejection 捕捉を備える。schedule registry 読込時にも空 path と絶対 path を fail-closed 検証する。`libs/core/trigger-runner.test.ts`、`managed-process.watch.test.ts`、pipeline scheduler 契約テスト計22件、core build、full build、`pipelines/qm02-trigger-validation.json` で検証。 |
