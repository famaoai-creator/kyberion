...
| R3-7 | **bare `pnpm <script>` 参照の検出が dead code**: `check_script_integrity.ts:100-102` は **存在する script 名だけ** `refs` に加えるため、削除済み script への参照を構造的に検出できない。`.github/PULL_REQUEST_TEMPLATE.md:77`(`pnpm check:catalogs`)、pre-PR checklist 12 箇所、live docs 54〜60 箇所が 3 ラウンド連続で残存                                                                                                                                                                                                                                                                                                                                                                                                                 | `scripts/check_script_integrity.ts:96-104`                                                                                                     | `scripts.has()` guard を外し `PNPM_BUILT_INS` とフラグ形だけ除外。`knowledge/` と `pipelines/**/*.md` を走査に追加                                                                                                             |
