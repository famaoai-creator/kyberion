...
- 2026-09-14: **HT-01 部分実装** — `presence/displays/presence-studio/hearing.ts` に Web アプリ要件 7 項目の純粋なレコード生成・会話ターン適用・充足率計算を追加。`missing_inputs` の別名を要件項目へ写像し、空ターンから回答を捏造しない契約を 3 テストで固定。UI/API の接続とキャンバス生成は未着手。
