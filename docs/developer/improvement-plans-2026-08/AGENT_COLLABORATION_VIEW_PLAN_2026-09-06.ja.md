...
| 3    | main 追従              | DONE(2026-09-07) | `ad8a1a452` `ec68d5499` `797e1e6e9` `36ea86652` `14e2b7969` `1631a26f8` `a67e79d80` | origin/main `bb69efdfc`(SX #711 マージ後)へ rebase し PR #712。main 側の既存失敗のうち依頼分を修正: blockchain-actuator の `readJsonLines` 型で `pnpm build` 復旧、retrospective の cost 期待値、process-boundary / fs-exception / package-boundary の登録儀式。自分の差分で増えた foundation-adoption・type-ratchet・cli-manifest(SX-05 ラチェット → script 追加撤回)・i18n baseline を解消。main と同一条件で全 suite を比較し、本ブランチの失敗集合は main の部分集合                                                                                    |
