...
- `README.md` のパネル表を 9 行に更新し、`pnpm tui:once` / `tui:dev` の記述を実在する `pnpm tui --once` に直す(package.json に `tui:once` を足す方が README 以外の文書とも整合するので、`"tui:once": "pnpm tui --once"` を追加する)。
...
| 2    | AC-05         | sonnet     | `presence/displays/terminal-hud/**`, `user-facing-vocabulary.json`(`tui` domain のみ), `vocabulary-keys.generated.ts`(生成), root `package.json`(`tui:once`)       | `pnpm test -- --suite tui`, `check:vocabulary-types`                    |
...
| 2    | AC-05         | DONE(2026-09-06) | `789b3a999` | パネル 9「連携」+ cockpit 1 行 + `tui:once`/`tui:dev` script。HUD は 7 日窓、root は最終イベント降順。peer-conversation transcript の detail 表示は agent_id → peer_id の対応が無く未実装(§5 に移す)                                                                                                                                                                                                                                                                                                                                                        |
...
既知の無関係な失敗(base `751d156eb` でも同じ): `libs/core/a2a-bridge.test.ts` と `tests/agent-collaboration-e2e.test.ts` の 14 件(`provider-capabilities.schema.json#/$defs/provenance` 欠落、Codex の reasoning コミット由来)、`tests/core-runtime-import-contract.test.ts` の `@agent/core/native-pptx-engine/text-metrics`(SX コミット `00b91dcf9` で export 追加も `libs/core/tsconfig.json` の include に無く dist 未出力)。
