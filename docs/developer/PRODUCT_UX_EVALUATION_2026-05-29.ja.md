...
| active mission count がノイズになる     | `kyberion vital --format=json` は `active_mission_count: 39` を返した。                                          | 新規ユーザには自分が何を片付けるべきか分からない。                |
...
| UX-1-4 | UI smoke gate                 | Chronos / Presence Studio の critical API が `pnpm test -- --suite ui-voice-browser-smoke` で検出される。 |
