---
title: AR 09 ACTUATOR COMMONIZATION
...
- 検証: `pnpm run typecheck`、`pnpm lint`、`pnpm run build:packages`、`pnpm run build:actuators`、`pnpm check -- --scope full --only op-registry`、`pnpm run test:actuators` を実行済み。
