---
title: AR 02 OP REGISTRY SINGLE SOURCE
...
**進捗メモ(2026-07-12 同日中、後続)**: ロングテール横展開を完了 — `DESCRIBE_OPS_SOURCES` は最終的に27アクチュエータ(+media は manifest 特例)で確定(agent/artifact/blockchain/build/calendar/email/presence/service/orchestrator/android/ios/vision/voice/meeting/media-generation/video-composition を追加)。CAPABILITIES op 表も discovery index からの生成へ切替済み(`sync_component_inventory.ts`)。working-memory-actuator / meeting-browser-driver は generic pipeline engine の dispatch switch を持たず専用 domain dispatch(manifest capabilities)のため本計画のスコープ外。
**2026-07-14 精査で確認**: `pnpm check:op-registry` 実行で "op registry is up to date"(ドリフト0)。CAPABILITIES_GUIDE.md も生成物であることを確認(Total Actuators: 30、`sync_component_inventory.ts`)。この日中フォローアップが STATUS.ja.md の AR 節に反映されずロングテール未了のまま陳腐化していたため訂正。
