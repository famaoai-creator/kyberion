...
- SR-01 にて入口を2つ追加: (1) `pnpm minutes:record --mission <ID>` — マイク録音 → VAD 区切り → バッチ STT → `meeting-followup.json` で minutes.md 自動生成(presence-studio の「会議を記録」ボタンからも同じ基盤)。(2) `meeting_participate --driver in-room` — ブラウザを使わない同席モード(下流の transcript→議事録は本計画の成果物をそのまま再利用)。
