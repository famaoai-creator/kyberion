...
4. **検証**: `pnpm app:preflight --platform ios` 実行(本環境で各項目が判定される)/ unit test(モックで pass/fail/fix 文言固定)/ `pnpm check -- --scope full --only catalogs`(manifest 変更後は `sync_component_inventory` 再生成)。
