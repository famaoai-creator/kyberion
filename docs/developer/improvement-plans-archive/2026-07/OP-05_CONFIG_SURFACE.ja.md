...
- **起動時の env 検証が部分的**: doctor(`doctor_core.ts:10-95`)はコマンド・ファイル・tier シンボリックリンク・運用メモリを確認するが、**env をスキーマ検証しない**。サブシステム別 preflight(`service_preflight.ts`、`environment-doctor.ts`、`bootstrap_environment.ts`)はあるが統一 env 契約でない。media runtime は `service:preflight -- --service media-generation` に統合されている。
