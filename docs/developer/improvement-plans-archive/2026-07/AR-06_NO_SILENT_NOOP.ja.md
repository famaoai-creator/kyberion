...
- **完了済み(一部)**: nested control の失敗を親 step へ伝播するようにし、`file-actuator` / `super-nerve` で control 内の失敗が success に化けないようにした。
- **完了済み(一部)**: `system-actuator` でも未知 op のエラー文に近い候補を付け、file と同じ teach パターンに寄せた。
- **完了済み(一部)**: `libs/actuators` 配下の source と `scripts/run_pipeline.ts` から `default: return ctx;` を検出する回帰テストを追加し、silent no-op の再導入を監視するようにした。
