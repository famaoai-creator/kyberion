---
title: MO 08 ARTIFACT REVIEW CLOSURE
...
12. 通常 task は `task_result.acceptance_evidence` で criterion、passed/failed、具体的 Evidence を1対1に束縛する。要約文の言語ではなく構造化対応で判定し、criterion 欠落、failed、Evidence 空は通過させない。
...
- [x] 通常 task は criterion-bound Evidence により言語非依存で完了でき、Evidence 不足時は review に留まる

## 5. Contract 影響

- Stable ADF v1、Actuator v1、CLI v1 の破壊的変更なし。
- `resolveMissionTeamReceiver` の optional filter は後方互換の additive change。
- `TaskResultBlock.acceptance_evidence` は optional な additive change。既存 `task_result` は引き続き validate できる。
...
## 9. Delivery dog-food で検出した lifecycle 欠陥

PR #561 の存在を GitHub CLI で確認し `delivery-report.md` を生成した delivery task でも、受入条件「リリース/PR が作成されている」と英語の `task_result` が逐語一致せず `review` に留まった。これを自由文の意味推定や無条件完了へ置き換えず、`task_result.acceptance_evidence` を追加した。worker は各 criterion の原文、`passed|failed`、具体的な検証または Artifact を返し、dispatcher は全 criterion に passed Evidence がある場合だけ完了させる。field は optional であり、既存結果との後方互換性を維持する。
