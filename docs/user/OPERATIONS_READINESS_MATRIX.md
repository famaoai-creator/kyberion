...
| Slack / iMessage / Discord / Telegram | Conditional | They work when the host app, permissions, and credentials are present. |
...
2. Run `pnpm setup:report`.
3. Run `pnpm doctor`.
4. Run `pnpm customer:list` if you use customer overlays.
5. Activate the customer you want with `pnpm customer:switch <slug>`.
6. Then work normally.
...
- iMessage, Telegram, and Discord integrations.
