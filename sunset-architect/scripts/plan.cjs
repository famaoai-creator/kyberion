  ...
  const timeline = [
    {
      phase: 'Announce',
      week: 1,
      action: `Announce deprecation of "${feature.name}" via changelog and in-app notice`,
      status: 'pending',
    },
    {
...
runSkill('sunset-architect', () => {
  ...
  const plans = features.map((f) => ({
    ...
    deprecationTimeline: planDeprecation(f),
  }));
  ...
  const result = {
    ...
    recommendations: [
      ...highRisk.map(
        (p) =>
          `[high] "${p.feature}" has ${p.impact.activeUsers} active users - needs careful migration plan`
      ),
      plans.length > 0
        ? `Total deprecation timeline: ~12 weeks per feature`
        : 'No features to sunset',
    ],
  };
