...
const ATTACK_VECTORS = [
  ...
  },
  {
    ...
    pattern: /exec\s*\(\s*[`'"].*\$\{|execSync.*\+/gi,
    severity: 'critical',
    category: 'Command Injection',
  },
  {
    id: 'path-traversal',
    pattern: /(?:readFile|createReadStream)\s*\(.*(?:req\.|params\.|query\.)/gi,
    severity: 'high',
  ...
  {
    id: 'no-csrf',
    pattern: /app\.(post|put|delete)\s*\(/gi,
    severity: 'medium',
    category: 'Missing CSRF Protection',
  },
  {
    id: 'debug-mode',
    pattern: /debug\s*[:=]\s*true|NODE_ENV.*development/gi,
    severity: 'medium',
...
function performRecon(dir) {
  ...
  for (const full of allFiles) {
    ...
    if (/config|setting/i.test(name) && /\.(json|yml|yaml|toml)$/.test(name))
      recon.configFiles.push(rel);
      ...
      criticalCount > 0
        ...
        : vulns.length > 5
          ...
          : vulns.length > 0
            ...
            : 'low',
    attackSurface: {
      endpoints: recon.publicEndpoints.length,
      configFiles: recon.configFiles.length,
      sensitiveFiles: recon.sensitiveFiles.length,
    },
    recommendations: [
      ...
      ...vulns
        ...
        .slice(0, 3)
        .map((v) => `[critical] ${v.category} in ${v.file}`),
    ],
  };
