  ...
  try {
    ...
    for (const line of lines) {
      ...
      if (methodMatch && currentPath) {
        ...
        paths[currentPath].push(methodMatch[1]);
      }
    }
    return {
      paths: Object.entries(paths).reduce((o, [p, methods]) => {
        o[p] = {};
        methods.forEach((m) => (o[p][m] = {}));
        return o;
      }, {}),
    };
  }
...
function diffAPIs(currentEndpoints, previousEndpoints) {
  ...
  for (const [key, ep] of currMap) {
    if (!prevMap.has(key)) changes.added.push(ep);
  }
  for (const [key, ep] of prevMap) {
    if (!currMap.has(key)) changes.removed.push(ep);
  }
  for (const [key, curr] of currMap) {
    const prev = prevMap.get(key);
    if (prev && (curr.parameters !== prev.parameters || curr.deprecated !== prev.deprecated))
      changes.modified.push({ ...curr, previousParams: prev.parameters });
  }
  return changes;
}
...
function detectBreakingChanges(changes) {
  ...
  for (const ep of changes.removed)
    breaking.push({ endpoint: `${ep.method} ${ep.path}`, type: 'removed', severity: 'breaking' });
  for (const ep of changes.modified) {
    if (ep.parameters < ep.previousParams)
      breaking.push({
...
runSkill('api-evolution-manager', () => {
  ...
  const result = {
    source: path.basename(argv.current),
    ...
    recommendations: [
      ...
      ...deprecated.map(
        (d) => `[deprecation] ${d.method} ${d.path} is deprecated - plan migration`
      ),
    ],
  };
