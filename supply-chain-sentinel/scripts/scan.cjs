...
const SUSPICIOUS_PATTERNS = [
  {
    pattern: /postinstall.*curl|wget|fetch/i,
    risk: 'Network call in postinstall script',
    severity: 'critical',
  },
  {
    pattern: /eval\s*\(\s*(?:Buffer|atob|decode)/i,
    risk: 'Obfuscated code execution',
    severity: 'critical',
  },
  { pattern: /child_process.*exec.*\$/i, risk: 'Dynamic command execution', severity: 'high' },
  {
    pattern: /process\.env\.(AWS|SECRET|TOKEN|KEY|PASSWORD)/i,
  ...
  if (fs.existsSync(pkgPath)) {
    ...
    for (const [name, version] of Object.entries(allDeps)) {
      components.push({
        ...
        ecosystem: 'npm',
        scope: pkg.dependencies?.[name] ? 'required' : 'development',
      });
    }
  }
  // Python dependencies
...
function checkProvenance(components) {
  ...
  for (const comp of components) {
    ...
    for (const known of knownPkgs) {
      if (
        comp.name !== known &&
        comp.name.includes(known) &&
        comp.name.length <= known.length + 3
      ) {
        concerns.push({
          package: comp.name,
          risk: 'typosquatting',
          detail: `Similar to known package "${known}"`,
          severity: 'high',
        });
      }
    }
    ...
    if (comp.version === 'latest' || comp.version === '*') {
      concerns.push({
        package: comp.name,
        risk: 'unpinned_version',
        detail: 'Using unpinned version - vulnerable to supply chain attacks',
        severity: 'medium',
      });
    }
  }
...
runSkill('supply-chain-sentinel', () => {
  ...
  const projectName = fs.existsSync(pkgPath)
    ? JSON.parse(fs.readFileSync(pkgPath, 'utf8')).name || 'unknown'
    : 'unknown';
  ...
  const riskScore = Math.min(
    100,
    provenance.filter((p) => p.severity === 'critical').length * 30 +
      provenance.filter((p) => p.severity === 'high').length * 15 +
      malicious.filter((m) => m.severity === 'critical').length * 40 +
      malicious.filter((m) => m.severity === 'high').length * 20
  );
