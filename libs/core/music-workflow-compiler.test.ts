...
describe('compileMusicGenerationADF', () => {
  it('builds an ACE-Step workflow from a music generation ADF', () => {
    ...
    expect(result.workflow['94']).toEqual(
      expect.objectContaining({
  ...
  it('falls back to a slugified filename prefix when none is provided', () => {
    const result = compileMusicGenerationADF({
      kind: 'music-generation-adf',
      version: '1.0.0',
      intent: 'Anniversary Song',
      style: {
        genre: 'country',
      },
      composition: {
    ...
    expect(result.resolved.filename_prefix).toBe('anniversary-song');
  });
  ...
  it('emits music-generation-adf that matches the schema', () => {
    ...
    const validate = compileSchemaFromPath(
      ajv,
      path.resolve(root, 'knowledge/product/schemas/music-generation-adf.schema.json')
    );
  ...
  it('rejects vocal workflows that omit provided lyrics', () => {
    ...
    ).toThrow('lyrics.text is required');
  });
});
