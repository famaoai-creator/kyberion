...
class ChildProcessAdapter implements TerminalAdapter {
  ...
  resize(_cols: number, _rows: number) {
    /* Emulated terminal does not support resize */
  }
  kill() {
    this.child.kill();
  }
...
class PtyRegistry {
      ...
      const check = () => {
        ...
        const match =
          typeof pattern === 'string'
            ? pattern === ''
              ? false
              : semanticBuffer.includes(pattern)
  ...
  public spawn(
    ...
    if (threadId && this.threadToSession.has(threadId)) {
      ...
      if (session && session.status === 'running') {
        logger.info(
          `[PTY_ENGINE] Re-attaching to existing thread: ${threadId} (Session: ${existingId})`
        );
        return existingId;
      }
    }
  ...
  public poll(
    ...
    offset?: number,
    limit?: number
  ): { output: string; nextOffset: number; total: number } {
    const session = this.sessions.get(id);
