...
describe('intent coverage contract', () => {
  it('keeps implemented intents aligned with standard-intents and outcomes', () => {
    ...
    for (const entry of coverage) {
      ...
      expect(intent?.resolution?.shape, `missing resolution shape for ${entry.intent_id}`).toBe(
        entry.execution_shape
      );

      for (const outcomeId of entry.outcome_ids || []) {
        expect(
          outcomes[outcomeId],
          `missing outcome ${outcomeId} for ${entry.intent_id}`
        ).toBeTruthy();
      }

      if (intent?.specialist_id) {
        expect(
          specialists[intent.specialist_id],
          `missing specialist ${intent.specialist_id} for ${entry.intent_id}`
        ).toBeTruthy();
      }

      const resolved = resolveWorkDesign({
        intentId: entry.intent_id,
        shape: entry.execution_shape as any,
        outcomeIds: entry.outcome_ids || [],
      });
      expect(resolved.outcomes.map((item) => item.id)).toEqual(
        expect.arrayContaining(entry.outcome_ids || [])
      );
    }
  });
  ...
  it('keeps implemented task-session and project-bootstrap intents classifiable from their first surface example', () => {
    ...
    for (const entry of coverage) {
      if (!['task_session', 'project_bootstrap'].includes(String(entry.execution_shape || '')))
        continue;
      const intent = standardIntents.find((candidate) => candidate.id === entry.intent_id);
      ...
      expect(
        classified,
        `could not classify implemented intent sample for ${entry.intent_id}`
      ).toBeTruthy();
  ...
  it('keeps every surface intent mapped to an intent-outcome pattern with valid outcomes', () => {
    ...
    for (const intent of standardIntents) {
      ...
      expect(
        pattern?.canonical_flow?.length || 0,
        `missing canonical flow for ${intent.id}`
      ).toBeGreaterThan(0);
      expect(
        pattern?.contract_layers?.length || 0,
        `missing contract layers for ${intent.id}`
      ).toBeGreaterThan(0);
      expect(
        pattern?.evidence?.length || 0,
        `missing evidence expectations for ${intent.id}`
      ).toBeGreaterThan(0);
      expect(
        pattern?.completion_criteria?.length || 0,
        `missing completion criteria for ${intent.id}`
      ).toBeGreaterThan(0);
      ...
      for (const outcomeId of intent.outcome_ids || []) {
        expect(
          pattern?.primary_outcome_ids || [],
          `pattern outcomes missing ${outcomeId} for ${intent.id}`
        ).toEqual(expect.arrayContaining([outcomeId]));
      }
    }
  });
  ...
  it('keeps advanced analysis intents mapped to a first-class analysis execution contract', () => {
    for (const intentId of [
      ...
      expect(
        contract?.required_bindings.length || 0,
        `missing required bindings for ${intentId}`
      ).toBeGreaterThan(0);
      expect(
        contract?.compiler_steps.length || 0,
        `missing compiler steps for ${intentId}`
      ).toBeGreaterThan(0);
      expect(
        contract?.evidence_outputs.length || 0,
        `missing evidence outputs for ${intentId}`
      ).toBeGreaterThan(0);
    }
  });
});
