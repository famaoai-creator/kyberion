...
export function isEnvironmentInitialized(): boolean {
  ...
  return (
    safeExistsSync(path.join(root, 'my-identity.json')) &&
...
export function buildSlackOnboardingBlocks(channel: string, threadTs: string): any[] {
  ...
  const blocks: any[] = [
    { type: 'section', text: { type: 'mrkdwn', text: `*Onboarding*\n${prompt.text}` } },
  ];
  if (prompt.field === 'language') {
    blocks.push({
      ...
      elements: ['日本語', 'English'].map((label) => ({
        ...
        value: serializeOnboardingAction({ channel, threadTs, field: prompt.field, answer: label }),
      })),
    });
    return blocks;
  }
    ...
    blocks.push({
      ...
      elements: [
        ...
        },
        {
          ...
          value: serializeOnboardingAction({ channel, threadTs, field: prompt.field }),
        },
      ],
    });
    return blocks;
  ...
  if (TEXT_MODAL_FIELDS.includes(prompt.field)) {
    ...
    if (answer) {
      blocks.push({
        type: 'context',
        elements: [{ type: 'mrkdwn', text: `Current value: ${answer}` }],
      });
    }
  }
  return blocks;
...
function persistOnboardingIdentity(state: OnboardingState): void {
  ...
  const interactionStyle = normalizeInteractionStyle(
    state.answers.interaction_style || 'Concierge'
  );
  ...
  withExecutionContext('sovereign_concierge', () => {
    ...
    safeWriteFile(
      path.join(root, 'agent-identity.json'),
      JSON.stringify(
        {
          agent_id: agentId,
          version: '1.0.0',
