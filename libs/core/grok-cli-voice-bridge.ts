...
export interface GrokCliVoiceBridgeOptions extends GrokCliBackendOptions {}

export class GrokCliVoiceBridge implements VoiceBridge {
  readonly name = 'grok-cli-text';
  private readonly options: GrokCliBackendOptions;

  constructor(options: GrokCliVoiceBridgeOptions = {}) {
    this.options = options;
  }

  async runRoleplaySession(input: RoleplaySessionInput): Promise<RoleplaySessionResult> {
    const result = (await runGrokCliQuery({
