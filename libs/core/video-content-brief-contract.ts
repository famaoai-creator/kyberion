...
export function compileVideoContentBriefToStoryboard(brief: VideoContentBrief): VideoStoryboard {
  ...
  const durationSec = clampDuration(
    brief.duration_sec || inferDuration(presentationMode, brief.content_type, beatPlan.length)
  );
  ...
  const logoPath = brief.design_system_ref.logo_path || tenantDesign?.logoPath || undefined;
  const tenantCssVars = isWebThemePack(tenantThemePack)
    ? webThemePackToCssVars(tenantThemePack)
    : {};

  let startSec = 0;
  const beats = beatPlan.map((entry, index) => {
    const duration =
      index === beatPlan.length - 1
        ? roundTo2(Math.max(0.1, durationSec - startSec))
        : durations[index];
    const designTokenHints = entry.designTokenHints(brief);
    const layoutVariant =
      typeof designTokenHints.layout_variant === 'string'
...
export function compileVideoStoryboardToNarratedVideoBrief(
  ...
  const presentationMode =
    storyboard.presentation_mode || normalizePresentationMode(undefined, storyboard.content_type);
  const hookText =
    hookBeat?.message ||
    hookBeat?.visual_intent ||
    hookBeat?.title ||
    storyboard.promise ||
  ...
  const backgroundColor =
    options.theme_background_color || storyboard.design_system_ref?.background_color || '#07111f';
  const fps = clampFps(options.timing?.fps || storyboard.design_system_ref?.fps || 30);
  const durationSec = clampDuration(
    options.timing?.duration_sec || sumStoryboardDuration(storyboard)
  );
...
function getBeatPlan(
  ...
  if (presentationMode === 'promo') {
    return [
      createBeatEntry(
        ...
        1,
        (brief) => brief.promise,
        (brief) => `Promise a sharper outcome for ${brief.audience}.`,
        () => 'Lead with the value spike.',
        (brief) => brief.promise,
        () => ['headline', 'offer'],
        () => ({
          layout_family: 'promo-spot',
          motion_profile: 'value-punch',
          beat_energy: 'high',
          typography_scale: 'expressive',
          overlay_density: 'compact',
          camera_distance: 'tight',
        })
      ),
      createBeatEntry(
        'value',
        ...
        'value',
        2,
        (brief) => brief.fixed_inputs?.message || brief.objective,
        () => 'Show the concrete value and immediate benefit.',
        () => 'Present the transformation clearly.',
      ...
      createBeatEntry(
        ...
        'proof',
        1,
        (brief) => brief.proof_points.join(' / '),
        () => 'Back the promise with proof and evidence.',
        () => 'Surface proof points quickly.',
        (brief) => brief.desired_takeaway,
        () => ['evidence', 'social-proof'],
        () => ({
          layout_family: 'promo-spot',
          motion_profile: 'artifact-reveal',
          beat_energy: 'medium',
      ...
      createBeatEntry(
        ...
        () => 'Ask for the next step explicitly.',
        (brief) => brief.desired_takeaway,
        () => ['cta', 'offer'],
        () => ({
          layout_family: 'promo-spot',
  ...
  if (presentationMode === 'vtuber') {
    return [
      createBeatEntry(
        ...
        1,
        (brief) => brief.promise,
        (brief) => `Open with a live persona cue for ${brief.audience}.`,
        () => 'Start with the avatar and the on-air premise.',
        (brief) => brief.promise,
        () => ['avatar', 'live-badge'],
        () => ({
      ...
      createBeatEntry(
        ...
        2,
        (brief) => brief.fixed_inputs?.message || brief.objective,
        () => 'Introduce the character, voice, and show context.',
        () => 'Make the presenter feel present.',
        (brief) => brief.objective,
        () => ['persona', 'stage'],
        () => ({
          layout_family: 'vtuber-stage',
          motion_profile: 'conversational',
          beat_energy: 'medium',
          typography_scale: 'balanced',
          overlay_density: 'medium',
          camera_distance: 'medium',
      ...
      createBeatEntry(
        ...
        'demo',
        2,
        (brief) => brief.content_requirements?.join(' / ') || brief.promise,
        () => 'Demonstrate the move or workflow live.',
        () => 'Show the act of doing, not just talking.',
        (brief) => brief.promise,
        () => ['screen', 'chat', 'process'],
        () => ({
          layout_family: 'vtuber-stage',
      ...
      createBeatEntry(
        ...
        1,
        (brief) => brief.desired_takeaway,
        () => 'Close with a community or next-action cue.',
        () => 'End on a clean audience-facing prompt.',
        (brief) => brief.desired_takeaway,
  ...
  if (normalized.includes('decision')) {
    return [
      createBeatEntry(
        ...
        1,
        (brief) => brief.promise,
        (brief) => `Frame the problem ${brief.audience} needs to solve.`,
        () => 'Guide the viewer to the decision context.',
        (brief) => brief.objective,
        () => ['decision-context'],
        () => ({ layout_family: 'decision', motion_profile: 'measured', beat_energy: 'medium' })
      ),
      createBeatEntry(
        'evidence',
        'Evidence',
        ...
        () => 'Use artifact-backed proof.',
        (brief) => brief.promise,
        () => ['evidence', 'artifact'],
        () => ({ layout_family: 'evidence', motion_profile: 'guided', beat_energy: 'medium' })
      ),
      createBeatEntry(
        'recommendation',
        'Recommendation',
        ...
        (brief) => brief.desired_takeaway,
        () => ['recommendation'],
        () => ({ layout_family: 'decision', motion_profile: 'guided-step', beat_energy: 'low' })
      ),
    ];
  }
  if (normalized.includes('product-walkthrough')) {
    return [
      createBeatEntry(
        ...
        1,
        (brief) => brief.promise,
        (brief) => `Promise a practical walkthrough for ${brief.audience}.`,
        () => 'Open with the product value.',
        (brief) => brief.promise,
        () => ['headline'],
        () => ({ layout_family: 'product', motion_profile: 'guided-step', beat_energy: 'high' })
      ),
      createBeatEntry(
        'context',
        'Context',
        ...
        'context',
        2,
        (brief) => brief.fixed_inputs?.use_case || brief.objective,
        () => 'Set the use case and constraints.',
        () => 'Show the operating context.',
        (brief) => brief.objective,
        () => ['context', 'use-case'],
        () => ({ layout_family: 'context', motion_profile: 'guided', beat_energy: 'medium' })
      ),
      createBeatEntry(
        'demo',
        'Demo',
        ...
        'demo',
        2,
        (brief) => brief.fixed_inputs?.message || brief.promise,
      ...
      createBeatEntry(
        ...
        1,
        (brief) => brief.proof_points.join(' / '),
        () => 'Anchor the walkthrough in proof.',
        () => 'Surface evidence and output.',
        (brief) => brief.desired_takeaway,
        () => ['artifact', 'evidence'],
        () => ({ layout_family: 'evidence', motion_profile: 'calm', beat_energy: 'medium' })
      ),
      createBeatEntry(
        'cta',
        'CTA',
        ...
        1,
        (brief) => brief.desired_takeaway,
        () => 'Close with a decisive action.',
        () => 'End on a clear next step.',
        (brief) => brief.desired_takeaway,
        () => ['next-action'],
        () => ({ layout_family: 'cta', motion_profile: 'minimal', beat_energy: 'low' })
      ),
    ];
  }
  if (normalized.includes('docs-demo') || presentationMode === 'howto') {
    return [
      createBeatEntry(
        ...
        1,
        (brief) => brief.promise,
        () => 'Set the expected outcome of the demo.',
        () => 'Introduce the demonstration intent.',
        (brief) => brief.promise,
        () => ['promise'],
        () => ({
          layout_family: 'howto-guide',
      ...
      createBeatEntry(
        ...
        'process',
        2,
        (brief) => brief.content_requirements?.join(' / ') || brief.objective,
        () => 'Walk through the steps that matter.',
        () => 'Show the ordered process.',
        (brief) => brief.objective,
        () => ['steps', 'procedure'],
        () => ({
          layout_family: 'howto-guide',
      ...
      createBeatEntry(
        ...
        1,
        (brief) => brief.proof_points.join(' / '),
        () => 'Surface the generated artifacts.',
        () => 'Highlight what gets produced.',
        (brief) => brief.promise,
        () => ['artifact', 'output'],
        () => ({
          layout_family: 'evidence',
      ...
      createBeatEntry(
        ...
        1,
        (brief) => brief.desired_takeaway,
        () => 'Validate the output and finish cleanly.',
        () => 'Close with verification.',
        (brief) => brief.desired_takeaway,
        () => ['validation'],
        () => ({
          layout_family: 'validation',
          motion_profile: 'minimal',
          beat_energy: 'low',
          typography_scale: 'balanced',
          overlay_density: 'compact',
  ...
  return [
    createBeatEntry(
      ...
      1,
      (brief) => brief.promise,
      (brief) => `State the value proposition for ${brief.audience}.`,
    ...
    createBeatEntry(
      ...
      1,
      (brief) => brief.desired_takeaway,
      () => 'Close with the requested action.',
...
function normalizePresentationMode(
  value: VideoPresentationMode | undefined,
  contentType?: string
...
function resolveStoryboardFormat(format?: VideoContentBrief['format']): {
  width: number;
  height: number;
  aspect_ratio: string;
} {
  ...
  const aspectRatio =
    normalizeAspectRatio(format?.aspect_ratio || `${width}:${height}`) || `${width}:${height}`;
  return { width, height, aspect_ratio: aspectRatio };
}

function resolveVideoTenantDesign(brief: VideoContentBrief) {
  const profile = brief.design_profile;
  if (!profile) return null;
  return resolveTenantDesign({
    rootDir: profile.root_dir,
...
function resolveAssetRefs(brief: VideoContentBrief, entry: BeatPlanEntry): string[] {
  ...
  if (
    brief.design_system_ref.hero_path &&
    (entry.role === 'feature' || entry.semantic === 'demo' || entry.semantic === 'process')
  ) {
...
function distributeDurations(total: number, weights: number[]): number[] {
  ...
  if (durations.length > 0) {
    durations[durations.length - 1] = roundTo2(
      Math.max(0.1, durations[durations.length - 1] + (total - roundedTotal))
    );
  }
  return durations;
}
