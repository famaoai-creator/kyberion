...
describe('browser extension bridge contracts', () => {
  ...
  it('requires a variable for every fill action', () => {
    ...
    expect(validateBrowserExtensionRecording(invalid).errors).toContain(
      'action step-2 must use a variable instead of a recorded value'
    );
  });
  ...
  it('requires explicit approval hashes for high-risk actions', () => {
    ...
    expect(execute.errors).toContain(
  ...
  it('rejects recordings whose human-readable text still carries raw PII', () => {
    ...
    expect(validateBrowserExtensionRecording(leakedName).errors).toContain(
      'action step-1 contains unredacted PII-like text'
    );
    expect(validateBrowserExtensionRecording(leakedSummary).errors).toContain(
      'action step-1 contains unredacted PII-like text'
    );
  });
  ...
  it('rejects a target name that carries captured body text instead of a label', () => {
    ...
    expect(validateBrowserExtensionRecording(bodyText).errors).toContain(
      'action step-1 target name looks like captured body text, not a label'
    );
  });
  ...
  it('blocks execute-mode preflight when the lease has expired', () => {
    ...
    const result = preflightBrowserExtensionSession({
      ...
      session: session({
        mode: 'execute',
        requested_operations: ['submit_form'],
        lease: issued.lease,
      }),
  ...
  it('crystallizes an approved recording into a schema-conformant draft pipeline', () => {
    ...
    const schema = JSON.parse(
      readFileSync(
        pathResolver.rootResolve('knowledge/product/schemas/browser-pipeline.schema.json'),
        'utf8'
      )
    );
  ...
  it('preserves the original browser recording op in the generated draft pipeline', () => {
    ...
    const draft = compileBrowserRecordingToPipeline(reviewed as any);
    expect(draft.steps[0].op).toBe('click');
    expect(draft.steps[0].params.original_op).toBe('select_ref');
    expect(draft._review_required).toContain(
  ...
  describe('persistBrowserExtensionReceipt (OP-H3)', () => {
    ...
    it('sanitizes receipt ids before writing the file path', () => {
      const mkdir = vi.spyOn(secureIo, 'safeMkdir').mockReturnValue(undefined as any);
      const write = vi.spyOn(secureIo, 'safeWriteFile').mockReturnValue(undefined);
      const receipt = buildBrowserExtensionReceipt({
        session: session({ mode: 'execute' }) as any,
      ...
      expect(result.errors).toHaveLength(0);
      expect(result.path).toContain('evil_receipt');
...
describe('segmented (multi-origin) execution', () => {
  function navAction(from: string, to: string) {
    return {
      ...
      captured_at: '2026-06-23T00:00:03.000Z',
      navigation: { from_origin: from, to_origin: to },
    };
  }
  function clickAction(id: string) {
    return {
      action_id: id,
      op: 'click_ref' as const,
      summary: 'click',
      risk: 'low' as const,
      captured_at: '2026-06-23T00:00:01.000Z',
  ...
  it('issues one origin-bound lease per segment', () => {
    ...
    expect(result.leases?.[1]).toMatchObject({
      segment_index: 1,
