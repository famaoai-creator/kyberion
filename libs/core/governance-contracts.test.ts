...
function readPayloadsFromDir(relativeDir: string): unknown[] {
  ...
  return safeReaddir(dir)
    ...
    .sort()
    .map((entry) =>
      JSON.parse(safeReadFile(path.join(dir, entry), { encoding: 'utf8' }) as string)
    );
}
...
const CASES: GovernanceSchemaCase[] = [
  ...
  {
    name: 'meeting-environment-policy',
  ...
  },
  {
    ...
    invalidPayload: {
      description:
        'Mission owner with final accountability, checkpoint, verify, and finish authority.',
    },
  },
  {
...
describe('governance contracts', () => {
  ...
  it('keeps deterministic golden scenarios in the canonical schema-managed catalogs', () => {
    ...
    const validate = compileSchemaFromPath(
      ...
      path.resolve(
        process.cwd(),
        'knowledge/product/schemas/mission-orchestration-scenario-pack.schema.json'
      )
    );
    expect(
      validate({
  ...
  it('keeps the canonical voice profile and surface provider directories aligned with their snapshots', () => {
    ...
    const voiceSnapshot = JSON.parse(
      safeReadFile(
        path.resolve(process.cwd(), 'knowledge/product/governance/voice-profile-registry.json'),
        { encoding: 'utf8' }
      ) as string
    );
    ...
    expect(
      voiceDirPayloads
        .map(
          (payload) =>
            (payload as { profiles?: Array<{ profile_id?: string }> }).profiles?.[0]?.profile_id
        )
        .sort()
    ).toEqual(
      (voiceSnapshot.profiles || [])
        .map((profile: { profile_id?: string }) => profile.profile_id)
        .sort()
    );
    ...
    const providerSchema = compileSchemaFromPath(
      ...
      path.resolve(
        process.cwd(),
        'knowledge/product/schemas/surface-provider-manifest-catalog.schema.json'
      )
    );
    ...
    const providerDirPayloads = readPayloadsFromDir(
    ...
    expect(
      providerDirPayloads
        .map((payload) => (payload as { entries?: Array<{ id?: string }> }).entries?.[0]?.id)
        .sort()
  ...
  it('keeps the canonical authority role directory aligned with the snapshot', () => {
    ...
    const authoritySnapshot = JSON.parse(
      safeReadFile(
        path.resolve(process.cwd(), 'knowledge/product/governance/authority-role-index.json'),
        { encoding: 'utf8' }
      ) as string
    ) as { authority_roles?: Record<string, unknown> };
    const authorityDirPayloads = readPayloadsFromDir(
  ...
  it('keeps the canonical service endpoints directory aligned with the snapshot', () => {
    ...
    const endpointSnapshot = JSON.parse(
      safeReadFile(
        path.resolve(process.cwd(), 'knowledge/product/orchestration/service-endpoints.json'),
        { encoding: 'utf8' }
      ) as string
    ) as { default_pattern?: string; services?: Record<string, unknown> };
    const endpointDirPayloads = readPayloadsFromDir(
      'knowledge/product/orchestration/service-endpoints'
    );
    ...
    expect(
      endpointDirPayloads
        .map(
          (payload) =>
            Object.keys((payload as { services?: Record<string, unknown> }).services || {})[0]
        )
        .sort()
  ...
  it('keeps the canonical service presets directory valid', () => {
    ...
    const presetDirPayloads = readPayloadsFromDir(
  ...
  it('keeps the canonical specialist catalog directory aligned with the snapshot', () => {
    ...
    const specialistSnapshot = JSON.parse(
      safeReadFile(
        path.resolve(process.cwd(), 'knowledge/product/orchestration/specialist-catalog.json'),
        { encoding: 'utf8' }
      ) as string
    ) as { version?: string; specialists?: Record<string, unknown> };
    const specialistDirPayloads = readPayloadsFromDir(
      'knowledge/product/orchestration/specialists'
    );
    ...
    expect(
      specialistDirPayloads
        .map(
          (payload) =>
            Object.keys((payload as { specialists?: Record<string, unknown> }).specialists || {})[0]
        )
        .sort()
  ...
  it('keeps the canonical voice engine directory aligned with the snapshot', () => {
    ...
    const engineSnapshot = JSON.parse(
      safeReadFile(
        path.resolve(process.cwd(), 'knowledge/product/governance/voice-engine-registry.json'),
        { encoding: 'utf8' }
      ) as string
    ...
    expect(
      engineDirPayloads
        .map(
          (payload) =>
            (payload as { engines?: Array<{ engine_id?: string }> }).engines?.[0]?.engine_id
        )
        .sort()
    ).toEqual(
  ...
  it('keeps the canonical actuator manifests aligned with the runtime snapshot', () => {
    const normalizeActuatorEntry = ({
      ...
      prerequisites_summary: _prerequisitesSummary,
      ...entry
    }: Record<string, unknown>) => entry;
    const catalog = loadActuatorManifestCatalog().map(normalizeActuatorEntry);
    const snapshot = JSON.parse(
      safeReadFile(
        path.resolve(process.cwd(), 'knowledge/product/orchestration/global_actuator_index.json'),
        { encoding: 'utf8' }
      ) as string
