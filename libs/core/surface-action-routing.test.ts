...
describe('surface action routing', () => {
  ...
  it('rejects prototype-bearing router output before validation', () => {
    expect(
      parseSurfaceActionRoutingDecision(
        '{"kind":"surface_action_routing","intent":"conversation","confidence":0.8,"__proto__":{"intent":"async_delegate"}}'
      )
    ).toBeNull();
  });
});
