...
function collectOverridePaths(
  ...
  const tenant = sanitizeIntentPathSegment(String(tenantId || ''));
  if (tenant && targetTier !== 'public') {
    paths.push(
      pathResolver.knowledge(`confidential/${tenant}/governance/track-policy-override.json`)
    );
  }
...
export async function resolveIntentToTrackPolicy(
  ...
  const lifecycleModels = effectivePolicy.lifecycle_models as
    Record<string, TrackLifecyclePolicy> | undefined;
  ...
  if (!lifecycleModel || !lifecyclePolicy)
    throw new Error(`Lifecycle policy not found: ${lifecycleModel || '(empty)'}`);
...
export function buildProjectTrackRecordFromPolicy(input: {
  ...
  const projectId =
    input.projectId?.trim() || `PRJ-${sanitizeIdFragment(input.policy.intent_id, 'INTENT')}`;
  ...
  return {
    ...
    summary:
      input.summary?.trim() ||
      input.policy.track_type_policy.description ||
      `Intent-provisioned track for ${input.policy.intent_id}.`,
    status: 'active',
...
export async function resolveIntentTrackGate(
  input: ResolveIntentTrackGateInput
): Promise<IntentTrackGateResult> {
  ...
  return {
    ...
    relationship: {
      track: {
        ...
        traceability_refs: [
          `intent:${policy.intent_id}`,
          `policy-lifecycle:${policy.lifecycle_model}`,
        ],
