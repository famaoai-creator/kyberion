...
export interface GrokCliIntentExtractorOptions extends GrokCliBackendOptions {}

export class GrokCliIntentExtractor implements IntentExtractor {
  readonly name = 'grok-cli';
  private readonly options: GrokCliBackendOptions;

  constructor(options: GrokCliIntentExtractorOptions = {}) {
  ...
  async extract(input: ExtractIntentInput): Promise<IntentBody> {
    ...
    const parsed = (await runGrokCliQuery({
