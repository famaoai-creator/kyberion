...
export interface GeminiCliVoiceBridgeOptions extends GeminiCliBackendOptions {}

export class GeminiCliVoiceBridge implements VoiceBridge {
  readonly name = 'gemini-cli-text';
  private readonly options: GeminiCliBackendOptions;

  constructor(options: GeminiCliVoiceBridgeOptions = {}) {
    this.options = options;
  }

  async runRoleplaySession(input: RoleplaySessionInput): Promise<RoleplaySessionResult> {
    const result = (await runGeminiCliQuery({
