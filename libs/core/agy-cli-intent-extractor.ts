...
export interface AgyCliIntentExtractorOptions extends AgyCliBackendOptions {}

export class AgyCliIntentExtractor implements IntentExtractor {
  readonly name = 'agy-cli';
  private readonly options: AgyCliBackendOptions;

  constructor(options: AgyCliIntentExtractorOptions = {}) {
  ...
  async extract(input: ExtractIntentInput): Promise<IntentBody> {
    ...
    const parsed = (await runAgyCliQuery({
