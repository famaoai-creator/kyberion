...
describe('Native PDF 2.0 Engine - Binary Generation', () => {
  ...
  it('should generate PDF without compression when disabled', async () => {
    ...
    await generateNativePdf(
      ...
      OUT,
      { compress: false, xmpMetadata: false }
    );
    const t = fs.readFileSync(OUT, 'binary');
  ...
  it('should encode Unicode text as UTF-16BE hex strings', async () => {
    ...
    await generateNativePdf(
      ...
      OUT,
      { compress: false, unicode: true, embed_cjk_font: false }
    );
    const t = fs.readFileSync(OUT, 'binary');
    ...
    await generateNativePdf(
      {
        ...
        aesthetic: {
          elements: [{ type: 'text', x: 123, y: 456, text: 'PrecisePosition', fontSize: 10 }],
        },
      } as any,
      OUT,
      { compress: false }
  ...
  it('should preserve Flate image soft masks as PNG alpha', async () => {
    ...
    fs.writeFileSync(
      ...
      Buffer.concat([
        ...
        ...objects.map((part) => (typeof part === 'string' ? Buffer.from(part, 'binary') : part)),
      ])
    );
    ...
    await generateNativePdf(
      {
        ...
        source: {
          format: 'markdown',
          body: 'Object stream extraction test',
          title: 'ObjStm Extract',
        },
      } as any,
      OUT,
      { compress: true, objectStreams: true }
    ...
    await generateNativePdf(
      {
        ...
        outlines: [
          { title: 'Chapter 1', pageIndex: 0 },
          {
            title: 'Chapter 2',
            pageIndex: 1,
            children: [{ title: 'Section 2.1', pageIndex: 1, top: 700 }],
          },
        ],
      } as any,
      OUT,
