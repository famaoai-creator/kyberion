...
/**
 * I18N-05: `locale` is the OOXML lang tag applied to <a:rPr> (spell-check
 * language, not visual style). Default 'ja-JP' preserves prior behavior.
 */
function textBodyXml(el: XlsxDrawingElement, locale = 'ja-JP'): string {
  if (el.txBodyXml) return el.txBodyXml;
  if (!el.text && (!el.textRuns || el.textRuns.length === 0)) return '';

  let xml =
    '<xdr:txBody><a:bodyPr vertOverflow="clip" horzOverflow="clip" wrap="square" rtlCol="0" anchor="t"/><a:lstStyle/>';
  ...
  if (el.textRuns && el.textRuns.length > 0) {
    ...
    for (const run of el.textRuns) {
      xml += '<a:r>';
      if (run.font) {
        xml += `<a:rPr lang="${locale}"`;
        if (run.font.size) xml += ` sz="${Math.round(run.font.size * 100)}"`;
        if (run.font.bold) xml += ' b="1"';
        if (run.font.italic) xml += ' i="1"';
        xml += '>';
        if (run.font.color?.rgb)
          xml += `<a:solidFill><a:srgbClr val="${run.font.color.rgb.replace('#', '')}"/></a:solidFill>`;
        if (run.font.name)
          xml += `<a:latin typeface="${escXml(run.font.name)}"/><a:ea typeface="${escXml(run.font.name)}"/>`;
        xml += '</a:rPr>';
      } else {
        xml += `<a:rPr lang="${locale}"/>`;
      }
    ...
    for (const line of lines) {
      xml += `<a:p><a:r><a:rPr lang="${locale}"/><a:t>${escXml(line)}</a:t></a:r></a:p>`;
    }
  }
...
function elementXml(el: XlsxDrawingElement, idCounter: number, locale = 'ja-JP'): string {
...
<xdr:wsDr xmlns:xdr="http://schemas.openxmlformats.org/drawingml/2006/spreadsheetDrawing" xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">`;
  ...
  for (const el of drawing.elements) {
    ...
    if (anchor.type === 'twoCellAnchor') {
      ...
      xml += elementXml(el, idCounter++, locale);
      xml += '<xdr:clientData/>';
      xml += '</xdr:twoCellAnchor>';
