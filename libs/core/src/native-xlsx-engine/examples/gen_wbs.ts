...
const cellXfs: XlsxCellStyle[] = [
  ...
  }, // 10: date header vertical
  {
    font: fonts[1],
    fill: fills[2],
    border: borders[1],
    alignment: { horizontal: 'center', vertical: 'center' },
  }, // 11: week header merged
  {
    font: fonts[3],
    fill: fills[11],
    border: borders[1],
...
rows.push({
  ...
  height: 30,
  customHeight: true,
  cells: [{ ref: 'A1', type: 's', value: 'Devin 評価プロジェクト WBS', styleIndex: 13 }],
});
  ...
  cells: [
    {
      ref: 'A2',
      type: 's',
      value: 'PJ オーナー: 市村　｜　期間: 2026/3/16 ～ 4/10　｜　成果物: 評価レポート',
      styleIndex: 14,
    },
  ],
});
...
rows.push({
  ...
  cells: [
    ...
    {
      ref: `${colRef(GANTT_START_COL + 1)}${rowNum}`,
      type: 's',
      value: '分析・レポート',
      styleIndex: 14,
    },
...
const outputPath = pathResolver.sharedTmp('native-xlsx/Devin評価PJ_WBS.xlsx');

async function main() {
  safeMkdir(pathResolver.sharedTmp('native-xlsx'), { recursive: true });
  ...
  console.log(
    `   Sheets: ${verify.sheets.length}, Rows: ${verify.sheets[0].rows.length}, MergeCells: ${verify.sheets[0].mergeCells.length}`
  );
  console.log(
    `   Columns: ${verify.sheets[0].columns.length}, Dimension: ${verify.sheets[0].dimension}`
  );
