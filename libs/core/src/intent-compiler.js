...
function synthesizeDeterministicSteps(intentDef) {
    const pipeline = intentDef?.pipeline;
...
export function compileIntent(intent, options) {
    ...
    let standardIntents = options?.standardIntents || [];
    if (standardIntents.length === 0 && safeExistsSync(intentsPath)) {
        try {
            const raw = safeReadFile(intentsPath, { encoding: 'utf8' });
    ...
    for (const si of standardIntents) {
        const triggers = si.triggers || si.trigger_keywords || si.keywords || [];
        const matched = triggers.some((t) => intentLower.includes(t.toLowerCase()));
        if (matched) {
 ...
 */
export function resolveIntentToSteps(intent, knowledgeHints) {
 ...
 */
export function buildPipelineGenerationPrompt(intent, hints, availableActuators) {
