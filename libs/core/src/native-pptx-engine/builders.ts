...
export function buildShape(el: PptxElement, id: number, rIdLink?: string): string {
  ...
  if (el.placeholderType) {
    const typeMap: any = { 'title': 'ctrTitle', 'body': 'body', 'subTitle': 'subTitle', 'sldNum': 'sldNum' };
  ...
  if (el.pXmlLst && el.pXmlLst.length > 0) {
    ...
    const rPrMatch = el.pXmlLst[0].match(/<a:rPr[^>]*>[\s\S]*?<\/a:rPr>/);
    if (rPrMatch) baseRPr = rPrMatch[0];
  }
  ...
  if (el.pXmlLst && el.pXmlLst.length > 0 && !isTextModified) {
    ...
    const bodyPr = el.bodyPrXml || '<a:bodyPr/>';
    const lstStyle = el.lstStyleXml || '<a:lstStyle/>';
    const alignMap: any = { 'left': 'l', 'center': 'ctr', 'right': 'r', 'justify': 'just' };
    ...
    runs.forEach(run => {
      let rPr = baseRPr; // Start with the original exact font/style
...
export function buildTable(el: PptxElement, id: number): string {
  ...
  el.rows.forEach((row, rIdx) => {
    ...
    for (let cIdx = 0; cIdx < colCount; cIdx++) {
      ...
      const safeText = cellText.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
      ...
      trXml += `<a:tc>
        <a:txBody>
          <a:bodyPr/>
          <a:lstStyle/>
          <a:p><a:r><a:rPr sz="1100" b="${isHeader ? '1' : '0'}"><a:solidFill><a:srgbClr val="${textColor}"/></a:solidFill></a:rPr><a:t>${safeText}</a:t></a:r></a:p>
        </a:txBody>
        <a:tcPr>
