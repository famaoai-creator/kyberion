import { z } from 'zod';
import { runAgyCliQuery, type AgyCliBackendOptions } from './agy-cli-backend.js';
import type {
...
export interface AgyCliVoiceBridgeOptions extends AgyCliBackendOptions {}

export class AgyCliVoiceBridge implements VoiceBridge {
  readonly name = 'agy-cli-text';
  private readonly options: AgyCliBackendOptions;

  constructor(options: AgyCliVoiceBridgeOptions = {}) {
    this.options = options;
  }

  async runRoleplaySession(input: RoleplaySessionInput): Promise<RoleplaySessionResult> {
    const result = (await runAgyCliQuery({
