...
import {
  getServiceOnboardingCatalogEntry,
  loadServiceOnboardingCatalog,
} from './service-onboarding-catalog.js';
...
describe('service-onboarding-catalog', () => {
  it('loads the canonical onboarding catalog from knowledge', () => {
    ...
    expect(catalog.services.map((entry) => entry.service_id)).toEqual([
      'comfyui',
      'whisper',
      'voice',
      'meeting',
  ...
  it('emits a catalog that satisfies the schema', () => {
    ...
    const schemaPath = path.join(
      ...
      'knowledge/product/schemas/service-onboarding-catalog.schema.json'
    );
