...
export function compileImageGenerationADF(adf: KyberionImageGenerationADF) {
  ...
  const filenamePrefix =
    ...
    slugify(adf.intent || 'image-generation', {
      maxLength: 48,
  ...
  return {
    workflow: {
      ...
      '3': {
        ...
        inputs: { text: adf.negative_prompt || '', clip: ['1', 1] },
      },
      '4': {
        class_type: 'EmptyLatentImage',
        inputs: { width: adf.canvas.width, height: adf.canvas.height, batch_size: 1 },
      },
      '5': {
        class_type: 'KSampler',
        inputs: {
...
export function compileVideoGenerationADF(adf: KyberionVideoGenerationADF) {
  ...
  if (!template) {
    throw new Error(
      'video-generation-adf currently requires either a supported named workflow_template or engine.base_workflow for embedded templates'
    );
  }
  const filenamePrefix =
    adf.output.filename_prefix ||
    slugify(adf.intent || 'video-generation', {
      maxLength: 48,
      fallback: 'kyberion-media',
    });
