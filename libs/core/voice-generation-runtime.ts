...
export interface VoiceGenerationJobApi {
  report(
    update: Partial<Omit<VoiceProgressPacket, 'kind' | 'job_id' | 'updated_at'>>
  ): VoiceProgressPacket;
  isCancelled(): boolean;
}
...
export class VoiceGenerationRuntime {
  ...
  public cancel(jobId: string): 'queued' | 'running' | null {
    ...
    if (queuedJob) {
      ...
      this.packets.set(
        jobId,
        this.buildPacket(jobId, 'cancelled', {
          progress: { current: 0, total: 1, percent: 0, unit: 'steps' },
  ...
  private buildPacket(
    jobId: string,
    status: VoiceJobStatus,
    data: Pick<VoiceProgressPacket, 'progress'> &
      Partial<Pick<VoiceProgressPacket, 'message' | 'artifact_refs'>>
  ): VoiceProgressPacket {
    return {
      kind: 'voice_progress_packet',
      job_id: jobId,
