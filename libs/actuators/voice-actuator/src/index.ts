...
import { transcribeVoiceSample } from './voice-transcribe.js';
import {
...
async function generateVoice(input: Record<string, any>): Promise<any> {
  ...
  // engine_id 'auto' (or none) = not named: the profile default, subject to
  // governed selection (voice-tts-engine seam); a named engine always wins.
  const namedEngineId = String(input.engine?.engine_id || '').trim();
  const engineNamed = Boolean(namedEngineId) && namedEngineId !== 'auto';
  const requestedEngineId =
    (engineNamed ? namedEngineId : String(profile.default_engine_id || '').trim()) ||
    profile.default_engine_id;
  const defaultEngine = getVoiceEngineRecord(profile.default_engine_id);
  let engine = resolveVoiceEngineForPlatform(requestedEngineId);
  ...
  const baselineBackend = resolveVoiceBackend(requestedEngineId);
  if (requiresPersonalVoice && (engine.kind !== 'voice_clone_service' || fallbackDetected)) {
    return {
      ...
      backend_id: baselineBackend.backend_id,
      backend_kind: baselineBackend.kind,
      backend_provider: baselineBackend.provider,
  ...
  const routing = routeVoiceEngine({
    ...
    explicit: engineNamed,
    purpose: typeof input.engine?.purpose === 'string' ? input.engine.purpose : undefined,
    language: input.rendering?.language,
    requires: {
      platform: process.platform,
      ...(deliveryMode === 'artifact' || deliveryMode === 'artifact_and_playback'
        ? { format: requestedFormat }
        : {}),
      ...(input.engine?.local_only === true ? { localOnly: true } : {}),
      ...voiceIdentityRequirement(engine, profile, requiresPersonalVoice),
    },
    // Reached only with a clone-capable, non-fallback engine when personal voice is required.
    personalVoiceLocked: requiresPersonalVoice,
  });
  for (const warning of routing.warnings) logger.warn(`[VOICE] ${warning}`);
  const baselineEngineId = engine.engine_id;
  engine = routing.engine;
  const backend =
    engine.engine_id === baselineEngineId ? baselineBackend : resolveVoiceBackend(engine.engine_id);
  ...
  const candidateEngineIds = routing.candidateEngineIds
    ? { candidateEngineIds: routing.candidateEngineIds }
    : {};
...
/**
 * Voice identity class the engine choice must preserve: the personal / cloned
 * voice stays cloned; a stock voice for a profile with reference samples must
 * not silently become a clone of those samples. No samples: no constraint.
 */
function voiceIdentityRequirement(
  engine: { supports: { voice_clone?: boolean; icl_ref_audio?: boolean } },
  profile: { sample_refs?: string[] },
  requiresPersonalVoice: boolean
): { identity?: 'clone' | 'stock' } {
  if (requiresPersonalVoice) return { identity: 'clone' };
  if (!Array.isArray(profile.sample_refs) || profile.sample_refs.length === 0) return {};
  return engine.supports.voice_clone && engine.supports.icl_ref_audio
    ? { identity: 'clone' }
    : { identity: 'stock' };
}
