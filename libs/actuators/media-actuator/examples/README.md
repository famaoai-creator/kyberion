...
`pdf_split` op は pypdf に依存します。利用前に、pypdf を含む Python runtime を `KYBERION_PYTHON_BIN` または `KYBERION_PYTHON` で指定してください。install 先は `knowledge/product/governance/dependency-placement-policy.md` に従います。

`pdf_encrypt` / AES 復号まで使う場合は `cryptography` も同じ runtime 側に必要です。
...
共通: `password?`（暗号化入力の復号用）, `out?`（未指定時は `active/shared/tmp/pdf-ops/<command>-<ts>.pdf`）, `timeout_ms?`, `export_as?`。`python` は `KYBERION_PYTHON_BIN` → `KYBERION_PYTHON` → `.venv/bin/python3` → `python3` の順で解決されます。入力・出力パスは Kyberion project root 内に制限されます。

**前提パッケージ:**

- `pypdf`: 全 op に必要
- `cryptography`: `pdf_encrypt`（AES-256）や AES 暗号化PDFの復号に必要
- いずれも `KYBERION_PYTHON_BIN` / `KYBERION_PYTHON` が指す runtime 側に入っている必要があります
