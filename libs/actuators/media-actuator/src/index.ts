...
function resolveMediaTenantDesign(
  ...
  const candidates = brandNames.map((value) => String(value || '').trim()).filter(Boolean);
...
function resolveLayoutTemplate(
  ...
  const customerId =
    String(slideData?.tenant_id || slideData?.client_key || '').trim() || undefined;
...
function resolveMediaDesignSystem(
  ...
  const resolveTenantOverride = (_system: any, designSystemId?: string) => {
    const brandCandidates = [
      bindingHints.branding?.brand_name || brief?.branding?.brand_name || brief?.client,
      brief?.payload?.client,
