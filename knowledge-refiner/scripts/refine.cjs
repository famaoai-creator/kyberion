...
function auditQuality(files) {
  ...
  for (const file of files) {
    ...
    if (file.words < 10 && file.size > 0)
      issues.push({ file: file.path, issue: 'Very low content (< 10 words)', severity: 'medium' });
    if (file.lines > 500)
      issues.push({
        file: file.path,
        issue: `Very large file (${file.lines} lines) - consider splitting`,
        severity: 'low',
      });
    if (!file.content.startsWith('#') && file.path.endsWith('.md'))
      issues.push({ file: file.path, issue: 'Markdown file missing heading', severity: 'low' });
  }
  return issues;
}
...
runSkill('knowledge-refiner', () => {
  ...
  if (duplicates.length > 0)
    result.recommendations.push(`${duplicates.length} duplicate(s) found - consider merging`);
  if (quality.filter((q) => q.severity === 'high').length > 0)
    result.recommendations.push('Empty files detected - remove or populate');
