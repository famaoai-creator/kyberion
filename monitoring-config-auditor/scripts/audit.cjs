...
const MONITORING_CHECKS = [
  {
    id: 'health-endpoint',
    patterns: ['/health', '/healthz', '/ready', '/readiness', '/liveness'],
    type: 'code',
    label: 'Health check endpoint',
  },
  {
    id: 'metrics-endpoint',
    patterns: ['/metrics', 'prometheus', 'prom-client', 'express-prometheus'],
    type: 'code',
    label: 'Metrics endpoint',
  },
  {
    id: 'logging',
    patterns: ['winston', 'pino', 'bunyan', 'morgan', 'log4js', 'console.error', 'logger'],
    type: 'dependency',
    label: 'Structured logging',
  },
  {
    id: 'alerting',
    patterns: ['alertmanager', 'pagerduty', 'opsgenie', 'slack-webhook', 'alert'],
    type: 'config',
    label: 'Alerting configuration',
  },
  {
    id: 'apm',
    patterns: ['datadog', 'new-relic', 'sentry', 'elastic-apm', '@opentelemetry'],
    type: 'dependency',
    label: 'APM / Tracing',
  },
  {
    id: 'uptime',
    patterns: ['uptime', 'pingdom', 'statuspage', 'betteruptime'],
    type: 'config',
  ...
  for (const check of MONITORING_CHECKS) {
    ...
    if (!found) {
      ...
      for (const full of allFiles) {
        if (!['.js', '.cjs', '.ts', '.yml', '.yaml', '.json'].includes(path.extname(full)))
          continue;
        try {
...
runSkill('monitoring-config-auditor', () => {
  ...
  const result = {
    directory: targetDir,
    score,
    status: score >= 80 ? 'well_monitored' : score >= 50 ? 'partial' : 'insufficient',
    checks: results,
