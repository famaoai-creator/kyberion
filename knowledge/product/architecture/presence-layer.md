...
Background presence services are managed by `scripts/surface_runtime.ts` and per-surface manifests under `knowledge/product/governance/surfaces/` with a compatibility snapshot in `active-surfaces.json`.
...
- `knowledge/product/presence/surface-query-providers.json` defines the public base
- `knowledge/product/governance/surface-query-overlay-catalog.json` lists the active overlay files and their roles/phases
- `knowledge/product/governance/surface-provider-manifest-catalog.json` lists the active provider manifests
...
4.  **Integrate**: Ensure your long-lived sensor or bridge is declared in `knowledge/product/governance/surfaces/*.json` for lifecycle management. The `active-surfaces.json` file is kept as a compatibility snapshot.
