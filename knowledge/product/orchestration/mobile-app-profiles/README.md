...
- profile 本体は `knowledge/product/orchestration/mobile-app-profiles/*.json`
- 一覧は `knowledge/product/orchestration/mobile-app-profiles/index.json`
- Android Actuator の high-level op は `app_profile` または `app_profile_from` で profile を受け取る
- schema は `knowledge/product/schemas/mobile-app-profile.schema.json`
