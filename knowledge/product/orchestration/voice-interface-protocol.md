...
| **Brain (LLM)**                       | Kyberion Reasoning Backend                                               | `KYBERION_REASONING_BACKEND` で切替（claude-cli / codex-cli / agy-cli / anthropic / nemotron-api / local）。Gemini CLI は明示的な Enterprise 設定向け。            |
...
| **VirtualAudioDeviceBridge**          | BlackHole 2ch / Loopback / PulseAudio route inventory                    | Owns device discovery and selects the concrete audio bus; stable CoreAudio UID is preferred over a display label.                                                  |
| **AudioOutputPort**                   | macOS CoreAudio AudioQueue                                               | Sends negotiated PCM directly to the selected device UID without changing macOS default output.                                                                    |
| **VirtualAudioOutputPlaybackBridge**  | legacy default-output-switch + afplay compatibility                      | Retained only for legacy speaker verification; it is not the BlackHole loopback path and emits a fallback warning.                                                 |
| **AudioBus**                          | bounded PCM transport                                                    | Owns input capture, output port, backpressure, health, cleanup, and device lease; the bus does not select physical speakers implicitly.                            |
