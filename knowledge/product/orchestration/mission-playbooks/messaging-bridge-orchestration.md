...
4.  **Reconciliation**: Kyberion reconciles `knowledge/product/governance/surfaces/*.json` through `scripts/surface_runtime.ts` / `service-actuator`, with `active-surfaces.json` as a compatibility snapshot.
