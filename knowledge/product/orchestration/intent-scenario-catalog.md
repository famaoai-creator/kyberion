...
| Voice profile registration    | `自分の声を使えるようにして` / `私の音声を学習して利用できるようにして` | `clone-my-voice` / `speak-with-my-voice`            | voice profile bundle                                          | `voice-actuator`, voice profile registry                                      | `collect-voice-samples` → `register-voice-profile` → `promote-voice-profile` → runtime store `active/shared/runtime/voice-profiles/<profile_id>/` |
...
| Google Workspace setup        | `Gmail / Calendar / Drive も連携して`                                   | `configure-organization-toolchain`                  | workspace collaboration bundle                                | `service-actuator` + `google-workspace` binding                               | if auth is missing, ask for the workspace account and CLI auth path                                                                               |
| Jira project setup            | `Jira も連携して`                                                       | `configure-organization-toolchain`                  | issue tracking bundle                                         | `service-actuator` + `jira` binding                                           | if project scope is unclear, ask for the Jira project key                                                                                         |
| Linear project setup          | `Linear のバックログも見たい`                                           | `configure-organization-toolchain`                  | product issue tracking bundle                                 | `service-actuator` + `linear` binding                                         | if team scope is unclear, ask for the Linear team identifier                                                                                      |
| Confluence knowledge setup    | `Confluence の runbook を整えたい`                                      | `configure-organization-toolchain`                  | documentation bundle                                          | `service-actuator` + `confluence` binding                                     | if space is unclear, ask for the Confluence space key                                                                                             |
| Notion workspace setup        | `Notion でまとめたい`                                                   | `configure-organization-toolchain`                  | lightweight knowledge bundle                                  | `service-actuator` + `notion` binding                                         | if workspace scope is unclear, ask for the Notion workspace URL                                                                                   |
| Zendesk support setup         | `Zendesk と連携して問い合わせを拾いたい`                                | `configure-organization-toolchain`                  | support ops bundle                                            | `service-actuator` + `zendesk` binding                                        | if support scope is unclear, ask for the Zendesk subdomain                                                                                        |
...
| Scheduled task inventory      | `スケジュールタスクの一覧を教えて`                                      | `inspect-generation-schedules`                      | generation schedule bundle                                    | `generation:schedule --action list` / `run_generation_schedule --action list` | if the user means calendar events instead, route to `schedule-read-agenda`                                                                        |
...
| Meeting operations            | `会議に入って要点を整理して`                                            | `meeting-operations`                                | meeting bundle                                                | `meeting-actuator`, `meeting-browser-driver`                                  | use `meeting-participation-request` for guided URL / purpose / role intake; if the provider is missing, ask for provider / mode / node            |
