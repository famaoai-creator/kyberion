...
- `create-my-avatar.json` - avatar onboarding template that captures a reference photo into the active profile root, registers it as the reference photo (`<profileRoot>/avatar.png`, identity `avatar_path`), and generates a stylised expression set from it into `<profileRoot>/avatar/draft` through the active host bridge or fallback provider (per-run consent for cloud / host-bridge providers). The reference photo and the displayed set stay separate; adopting the draft is an explicit step (concierge Settings > Voice > avatar > "Use this avatar").
