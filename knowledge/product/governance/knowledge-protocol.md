...
scope: global
authority: policy
phase: [onboarding, alignment, execution, review]
role_affinity: [knowledge_steward, ecosystem_architect, mission_controller]
applies_to: [knowledge, tiering, missions]
...
本モノレポの全 Actuator / Agent / Mission が遵守すべき、ナレッジの階層構造と取り扱い基準。
...
2. **Confidential Tier (`knowledge/confidential/`, `active/missions/confidential/`)**: 会社・プロジェクト共有の秘密。外部Git管理。
   - **Capability-Specific**: `.../actuators/<actuator-name>/`
...
- **ミッション・ティアの継承**: ミッションの `knowledge_injections` は各要素が `tier`(+ 該当時は `project`、検索用の `domains`/`tags`)を明示的に宣言する。パスのハードコードではなく宣言された `tier` を直接読むため、`knowledge/` 配下のディレクトリ再編に影響されない。宣言中に上位ティアが含まれる場合、ミッション自体の実行ティアも自動的に引き上げられる。
- **独立履歴 (Micro-Git)**: 各ミッションはディレクトリ内に独自の `.git` を持ち、メインリポジトリの歴史から物理的に隔離される。これにより、試行錯誤の過程や機密データがメインリポジトリに混入することを防止する。

## 3. Capability の行動原則 (Core Logic)

- **優先順位 (Precedence)**: 同じ定義がある場合、以下の順で優先適用する。
  1. **Personal Tier** (個人の設定が最優先)
  2. **Confidential Tier (Client-Specific)** (クライアント固有設定)
  3. **Confidential Tier (Capability-Specific/General)** (会社標準)
  4. **Public Tier** (一般標準)
- **透過的参照**: 実行時、Actuator / Agent は自動的に全 Tier を統合して最適なコンテキストを構築する。
