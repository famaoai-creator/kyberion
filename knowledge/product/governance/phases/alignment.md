...
2. **Context Ranking**: Run `node dist/scripts/context_ranker.js --intent "..." --role "..." --limit 7 --explain` (or `pnpm knowledge:rank ...`) yourself to identify the TOP-7 most relevant knowledge files and verify the active tenant/org/project roots and scope exclusions. Supply `--tenant`, `--organization`, `--project`, `--mission`, or establish them with `pnpm scope use` when ranking confidential knowledge. This is a **manual step the agent performs**, not something `surface-runtime-orchestrator.ts` invokes automatically — there is no code path that shells out to `context_ranker` on your behalf. Skipping it means Alignment proceeds without the noise-reduction pass.
