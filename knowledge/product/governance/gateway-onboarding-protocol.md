...
- **実務**: `surface-registration` の config mission を system scope と preflight ref 付きで起票する。正本 JSON を直接編集せず、適用後に `active-surfaces.json` を生成する。
- **ガバナンス**: system scope の surface exposure は approval-store の human decision と payload fingerprint の一致を要求し、適用後に `runtimeSupervisor` の監視下へ入る。
...
_Status: Living Governance Document for Multi-Channel Expansion_
