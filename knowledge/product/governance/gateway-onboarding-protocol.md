...
- **実務**: `knowledge/product/governance/surfaces/<surface-id>.json` にサービスID、種別（gateway）、起動コマンド、ポート等を追記する。`active-surfaces.json` は compatibility snapshot として自動生成される。
...
- **実務**: `pnpm surfaces:setup` で認証準備を確認した後に `pnpm surfaces:reconcile` を実行。システムは「コード」「設定」「鍵」が全て揃っていることを検証し、バックグラウンドプロセスを立ち上げる。
