...
1. Add a new profile under `llm.profiles` in [`wisdom-policy.json`](/Users/famao/kyberion/knowledge/product/governance/wisdom-policy.json).
...
- Mission distills write to `knowledge/product/evolution/`; older `knowledge/incidents/` files are kept only for read compatibility.
