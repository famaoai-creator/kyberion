...
For multilingual operator input:

- let the LLM normalize free-form requests into English-first canonical intent
- do not create parallel multilingual execution contracts
- keep schemas, archetypes, catalogs, and pipeline contracts canonical in English

For multilingual operator output:

- localize stable UI labels through the governed vocabulary catalog
- allow summaries, previews, and explanations to be generated by the LLM in the operator's language
- use English as the fallback whenever a localized fixed label does not exist
