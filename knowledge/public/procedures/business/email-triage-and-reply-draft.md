...
1. Check Gmail auth status with `pnpm kyberion email status` or `pnpm email:workflow status`.
2. Generate a reply draft with `pnpm kyberion email draft --triage-file active/shared/tmp/email-inbox-triage.md`.
3. Inspect the latest stored draft with `pnpm kyberion email latest-draft`.
4. Create a Gmail draft or send an approved message with `pnpm kyberion email deliver ...`.
5. If you want to archive repeated unread inbox senders, run `pnpm kyberion email archive-inbox --apply` after reviewing the preview output.
