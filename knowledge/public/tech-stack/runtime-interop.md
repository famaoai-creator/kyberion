...
All development must happen in `.ts` files. The `dist/` folder remains the runtime output, and `pnpm run build` plus `pnpm run check -- --scope pr --only esm` are the standard integrity gates.
