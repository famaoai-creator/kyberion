...
| :------------------------------------ | :----------------------- | :------------------------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------- |
| **`github`**                          | Dev / BTS                | `GITHUB_TOKEN` / `ACCESS_TOKEN`                                      | Canonical REST preset (`service-presets/github.json`): issues, PRs, reviews (capture + write-gated submit), Actions.                       | 正本の REST preset。Issue/PR/review 取得と承認付き review 投稿、Actions。                                                              |
| **`github-mcp`** (deprecated example) | Dev / BTS                | `GITHUB_TOKEN` / `ACCESS_TOKEN`                                      | External-MCP fixture for the retired `@modelcontextprotocol/server-github` 3-tool server. Do not use for daily GitHub work — use `github`. | 退役した MCP パッケージの外部 MCP 配線例。日常の GitHub 作業には使わない。                                                             |
