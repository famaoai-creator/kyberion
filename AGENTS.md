...
- **Organization & projects**: mutate the organization operating model and managed projects only via their governed facades (`pnpm organization`, `pnpm project`, `pnpm org`) — never by editing state under `active/organizations/` directly. Work items carry the canonical context chain (`tenant_slug → organization_id → project_id → mission_id → task_id` + `work_shape`) — one order for containment, storage and serialization, declared in `libs/core/entity-scope.ts` and explained in [entity-scope-hierarchy](./knowledge/product/architecture/entity-scope-hierarchy.md). Set typed `context` on creation instead of encoding it in labels. Tier and partition names (`public`, `confidential`, `personal`, `shared`) are never tenants (`RESERVED_SCOPE_NAMES`), and the `customer/{slug}/` overlay is a stance, not a scope in this chain → [stance-tenant-customer-model](./knowledge/product/architecture/stance-tenant-customer-model.md). → [ORGANIZATION_VIEW_SCOPE_ARCHITECTURE](./docs/developer/improvement-plans-2026-07/ORGANIZATION_VIEW_SCOPE_ARCHITECTURE_2026-08-04.ja.md)
