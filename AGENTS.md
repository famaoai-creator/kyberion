...
- **Reasoning backend**: prefer `KYBERION_REASONING_BACKEND=claude-cli` (local `claude` CLI, no API key) → `grok-cli` (local Grok Build CLI `grok`, OAuth via `grok login`) → `anthropic` (`ANTHROPIC_API_KEY`) → `stub` (offline/deterministic tests). Auto-discovery also considers installed `codex` / `agy` / `grok` CLIs. Divergent-thinking `wisdom:*` ops need a non-stub backend.
