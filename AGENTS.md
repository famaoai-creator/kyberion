...
- **Organization & projects**: mutate the organization operating model and managed projects only via their governed facades (`pnpm organization`, `pnpm project`, `pnpm org`) — never by editing state under `active/organizations/` directly. Work items carry a canonical context chain (`organization_id → tenant_slug → mission_id → project_id → task_id` + `work_shape`); this is field-listing order, not a containment claim; the canonical containment hierarchy is defined in [entity-scope-hierarchy](./knowledge/product/architecture/entity-scope-hierarchy.md). Set typed `context` on creation instead of encoding it in labels. → [ORGANIZATION_VIEW_SCOPE_ARCHITECTURE](./docs/developer/improvement-plans-2026-07/ORGANIZATION_VIEW_SCOPE_ARCHITECTURE_2026-08-04.ja.md)
