...
const argv = createStandardYargs()
  .option('action', {
    ...
    type: 'string',
    default: 'status',
    choices: ['status', 'send', 'summary', 'alert'],
    description: 'Action',
  })
  .option('channel', {
    alias: 'c',
...
function checkWebhook() {
  ...
  for (const p of paths) {
    if (fs.existsSync(p)) {
      try {
        return { configured: true, ...JSON.parse(fs.readFileSync(p, 'utf8')) };
      } catch (_e) {}
    }
  }
  ...
  if (action === 'alert') {
    const data =
      input && fs.existsSync(input)
        ...
        : { level: 'warning', message: input || 'Alert' };
    const emoji =
      data.level === 'critical'
        ? ':rotating_light:'
        : data.level === 'warning'
    ...
    message.blocks = [
      {
        type: 'header',
        text: { type: 'plain_text', text: `${emoji} ${data.level?.toUpperCase() || 'ALERT'}` },
      },
    ...
    message.blocks = [
      { type: 'header', text: { type: 'plain_text', text: data.title || 'Summary Report' } },
      {
        ...
        text: {
          type: 'mrkdwn',
          text: data.content || JSON.stringify(data, null, 2).substring(0, 500),
        },
      },
    ];
  } else {
    message.blocks = [
      { type: 'section', text: { type: 'mrkdwn', text: input || 'Hello from Gemini Skills!' } },
    ];
  }
  return message;
}
...
runSkill('slack-communicator-pro', () => {
  ...
  const result = {
    ...
    message,
    recommendations: !webhook.configured
      ? ['Create Slack webhook config at knowledge/personal/slack-webhook.json']
      : [],
  };
