...
const VULNERABLE_CRYPTO = [
  {
    pattern: /\b(RSA|rsa)\b/g,
    algorithm: 'RSA',
    risk: 'high',
    replacement: 'ML-KEM (CRYSTALS-Kyber)',
  },
  {
    pattern: /\b(ECDSA|ecdsa|secp256k1|secp384r1|P-256|P-384)\b/g,
    algorithm: 'ECDSA/ECC',
    risk: 'high',
    replacement: 'ML-DSA (CRYSTALS-Dilithium)',
  },
  {
    pattern: /\b(Diffie.Hellman|DH|ECDH|ecdh)\b/gi,
  ...
  {
    pattern: /\b(AES-128|aes128)\b/gi,
    algorithm: 'AES-128',
    risk: 'medium',
    replacement: 'AES-256 (Grover resistance)',
  },
  { pattern: /\b(MD5|md5)\b/g, algorithm: 'MD5', risk: 'critical', replacement: 'SHA-3 or SHAKE' },
  {
    pattern: /\b(SHA-1|sha1)\b/gi,
    algorithm: 'SHA-1',
    risk: 'critical',
    replacement: 'SHA-3 or SHA-256+',
  },
];
      ...
      for (const vuln of VULNERABLE_CRYPTO) {
        ...
        if (matches)
          findings.push({
            ...
            risk: vuln.risk,
            pqcReplacement: vuln.replacement,
...
function assessMigrationEffort(findings) {
  ...
  for (const f of findings) {
    if (!byAlgorithm[f.algorithm])
      byAlgorithm[f.algorithm] = {
        files: 0,
        occurrences: 0,
        replacement: f.pqcReplacement,
        risk: f.risk,
      };
  ...
  return effort.sort((a, b) => {
    const r = { critical: 0, high: 1, medium: 2 };
    return (r[a.risk] || 3) - (r[b.risk] || 3);
  });
}
...
runSkill('post-quantum-shield', () => {
  ...
  const riskScore = findings.reduce(
    (s, f) => s + (f.risk === 'critical' ? 30 : f.risk === 'high' ? 15 : 5),
    0
  );
  const result = {
    directory: targetDir,
    quantumVulnerable: findings.length > 0,
    ...
    recommendations: migration
      ...
      .map(
        (m) =>
          `[${m.risk}] Replace ${m.algorithm} with ${m.replacement} (${m.files} files, effort: ${m.estimatedEffort})`
      ),
  };
