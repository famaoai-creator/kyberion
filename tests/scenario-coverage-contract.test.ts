...
describe('scenario coverage contracts', () => {
  it('keeps the documented scenario references pointing at real playbooks and pipelines', () => {
    const scenarioDoc = safeReadFile(pathResolver.rootResolve('docs/SCENARIOS.md'), {
      encoding: 'utf8',
    }) as string;
    ...
    expect(
      safeExistsSync(
        pathResolver.rootResolve(
          'knowledge/product/orchestration/mission-playbooks/product-audit.md'
        )
      )
    ).toBe(true);
    expect(safeExistsSync(pathResolver.rootResolve('pipelines/ceo-strategic-report.json'))).toBe(
      true
    );
    expect(
      safeExistsSync(
        pathResolver.rootResolve(
          'knowledge/product/orchestration/mission-playbooks/ceo-strategy.md'
        )
      )
    ).toBe(true);
  });
  ...
  it('keeps the scenario packs populated with the flagship multi-actuator scenarios', () => {
    const orchestrationPack = readJson(
    ...
    const ids = new Set(
      (orchestrationPack.scenarios || []).map((scenario) => scenario.scenario_id).filter(Boolean)
    );
    ...
    const notes = (orchestrationPack.scenarios || [])
      .map((scenario) => scenario.notes || '')
      .join('\n');
  ...
  it('keeps the canonical actuator archetype targets backed by manifests', () => {
    ...
    const archetypePack = readJson(
      'knowledge/product/orchestration/actuator-request-archetypes.json'
    ) as {
