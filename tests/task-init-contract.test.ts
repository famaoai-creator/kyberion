...
describe('task init contract', () => {
  ...
  it('writes a profile from non-interactive answers and prints the next command', async () => {
    ...
    await main(['daily-email-triage', '--answers-file', ANSWERS_FILE], (value) =>
      output.push(value)
    );
    ...
    expect(rendered).toContain(
      'Created profile: knowledge/personal/task-profiles/daily-email-triage.json'
    );
    expect(rendered).toContain('Next: pnpm task:run daily-email-triage');
  });

  it('prints an answer template without writing a profile', async () => {
    const output: unknown[] = [];

    await main(['daily-email-triage', '--print-template'], (value) => output.push(value));

    expect(safeExistsSync(PROFILE_PATH)).toBe(false);
    const template = JSON.parse(String(output[0])) as Record<string, string>;
  ...
  it('fails gracefully when the scenario id is unknown', async () => {
    await expect(main(['unknown-scenario', '--answers-json', '{}'])).rejects.toThrow(
  ...
  it('rejects profile outputs outside the personal knowledge tier', async () => {
    ...
    const original = JSON.parse(
      safeReadFile(
        pathResolver.rootResolve('knowledge/product/task-scenarios/daily-email-triage.json'),
        { encoding: 'utf8' }
      ) as string
    );
    original.first_run.profile_output = 'knowledge/product/task-profiles/not-allowed.json';
    safeWriteFile(
      pathResolver.rootResolve('active/shared/tmp/task-init-scenarios/daily-email-triage.json'),
    ...
    try {
      await expect(main(['daily-email-triage', '--answers-file', ANSWERS_FILE])).rejects.toThrow(
        'Profile output must stay under knowledge/personal/'
      );
    } finally {
