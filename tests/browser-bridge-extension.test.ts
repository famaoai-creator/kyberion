...
function defaultNative(payload: any) {
  if (payload.type === 'request_execution') {
    return { ok: true, status: 'authorized', lease: { lease_id: 'L1', issued_at: '2026-06-23T00:00:00.000Z', expires_at: '2999-01-01T00:00:00.000Z', approved_step_hashes: [] } };
  }
  if (payload.type === 'submit_receipt') return { ok: true, status: 'recorded', receipt_id: payload.receipt.receipt_id };
  if (payload.type === 'preflight') return { ok: true, status: 'ready_for_review', approval_required: false };
  return { ok: true };
}
...
async function createHarness() {
  ...
  const chrome: any = {
    ...
    runtime: {
      ...
      lastError: undefined,
      sendNativeMessage: vi.fn((_name: string, payload: any, cb: (response: any) => void) => {
        Promise.resolve(hooks.native(payload)).then((response) => {
          chrome.runtime.lastError = hooks.nativeError ? { message: hooks.nativeError } : undefined;
          cb(response);
        });
      }),
    },
    tabs: {
      ...
      sendMessage: vi.fn(async (_tabId: number, message: any) => {
        ...
        if (message.type === 'bridge:execute-step') return hooks.execute(message.step, message.value);
        return { ok: true };
      }),
    },
    ...
    storage: {
      ...
      session: {
        get: vi.fn(async (key: string) => ({ [key]: store[key] === undefined ? undefined : structuredClone(store[key]) })),
        set: vi.fn(async (value: Record<string, any>) => { for (const k of Object.keys(value)) store[k] = structuredClone(value[k]); }),
      },
    },
  };
...
function submitEvent(summary: string, ref = '@form_1_1') {
  return { op: 'submit_form', summary, target: { ref, role: 'form', name: 'Checkout', snapshot_hash: HASH } };
}

async function approveAllAndFinalize(harness: Awaited<ReturnType<typeof createHarness>>, events: any[]) {
  ...
  for (const action of stopped.draft.actions) {
    await harness.send({ type: 'bridge:review-action', actionId: action.action_id, decision: 'approved' });
  }
...
describe('Browser Bridge extension state transitions', () => {
  ...
  it('auto-resumes recording after a same-origin navigation completes', async () => {
    ...
    await harness.update(harness.tab.id, { status: 'complete' }, { url: 'https://example.com/next', title: 'Next' });
  ...
  it('continues across a cross-origin navigation by recording a navigate handoff (segmented recording)', async () => {
    ...
    await harness.send({ type: 'bridge:start-recording' });
    await harness.update(harness.tab.id, { status: 'loading' });
    await harness.update(harness.tab.id, { status: 'complete' }, { url: 'https://app.example.com/next', title: 'Sub' });
  ...
  it('keeps every step when a burst of record-events arrives concurrently', async () => {
    ...
    await Promise.all(
      Array.from({ length: 6 }, (_unused, i) =>
        harness.send({ type: 'bridge:record-event', event: recordedEvent(`操作${i}`, `@button_${i}_1`) }, { tab: harness.tab }),
      ),
    );
