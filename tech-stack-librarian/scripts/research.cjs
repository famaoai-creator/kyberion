  ...
  if (fs.existsSync(pkgPath)) {
    try {
      ...
      for (const dep of Object.keys(allDeps)) {
        ...
        if (BEST_PRACTICES[key])
          stack.push({ name: dep, version: allDeps[dep], ...BEST_PRACTICES[key] });
      }
    } catch (_e) {}
  }
  if (fs.existsSync(path.join(dir, 'tsconfig.json')))
    stack.push({ name: 'typescript', version: 'detected', ...BEST_PRACTICES.typescript });
  if (fs.existsSync(path.join(dir, 'Dockerfile')))
    stack.push({ name: 'docker', version: 'detected', ...BEST_PRACTICES.docker });
  if (fs.existsSync(path.join(dir, 'terraform')) || fs.existsSync(path.join(dir, 'main.tf')))
    stack.push({ name: 'terraform', version: 'detected', ...BEST_PRACTICES.terraform });
  return stack;
}
...
runSkill('tech-stack-librarian', () => {
  ...
  const result = {
    directory: targetDir,
    technologiesDetected: stack.length,
    ...
    recommendations: stack
      .flatMap((s) => s.practices.slice(0, 2).map((p) => `[${s.name}] ${p}`))
      .slice(0, 10),
  };
