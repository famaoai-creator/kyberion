...
const argv = createStandardYargs()
  ...
  .option('standard', {
    ...
    type: 'string',
    default: 'soc2',
    choices: ['soc2', 'iso27001', 'hipaa', 'gdpr', 'all'],
        ...
        checks: [
          {
            ...
            patterns: ['monitor', 'alert', 'prometheus', 'grafana', 'datadog'],
          },
          {
            id: 'CC7.2',
            label: 'Incident response',
            patterns: ['incident', 'runbook', 'escalat', 'on-call'],
          },
          {
            id: 'CC7.3',
            label: 'Backup & recovery',
            patterns: ['backup', 'restore', 'disaster-recovery', 'dr'],
          },
        ],
      },
      {
        ...
        checks: [
          ...
          },
          {
            ...
            patterns: ['CODEOWNERS', 'pull_request_template', 'CONTRIBUTING'],
          },
        ],
      },
    ],
        ...
        checks: [
          {
            ...
            patterns: ['SECURITY.md', 'security-policy'],
          },
        ],
      },
      {
        ...
        checks: [
          ...
          },
          {
            ...
            patterns: ['package-lock.json', 'yarn.lock', 'Pipfile.lock'],
          },
        ],
      },
      {
        ...
        checks: [
          { id: 'A12.1', label: 'Logging', patterns: ['logger', 'winston', 'pino', 'log4j'] },
          {
            ...
            patterns: ['snyk', 'dependabot', 'npm audit', 'security-scanner'],
          },
        ],
      },
    ],
    ...
    categories: [
      {
        ...
        checks: [
          { id: 'PHI.1', label: 'Data encryption', patterns: ['encrypt', 'tls', 'ssl', 'https'] },
          {
            ...
            patterns: ['rbac', 'role', 'permission', 'auth'],
          },
        ...
        checks: [
          {
            ...
            patterns: ['risk', 'assessment', 'security-scan'],
          },
          {
            id: 'ADMIN.2',
            label: 'Contingency plan',
            patterns: ['backup', 'disaster', 'contingency', 'recovery'],
          },
        ],
      },
    ],
        ...
        checks: [
          {
            ...
            patterns: ['privacy', 'consent', 'gdpr', 'data-protection'],
          },
          {
            id: 'ART25.2',
            label: 'Data minimization',
            patterns: ['minimal', 'necessary', 'purpose-limit'],
          },
        ],
      },
      {
        ...
        checks: [
          { id: 'ART32.1', label: 'Encryption', patterns: ['encrypt', 'hash', 'bcrypt', 'argon'] },
          {
            ...
            patterns: ['anonymiz', 'pseudonym', 'mask', 'redact'],
          },
        ],
      },
    ],
  ...
  function search(d, depth) {
    ...
    try {
      for (const e of fs.readdirSync(d, { withFileTypes: true })) {
        ...
        if (
          !['.js', '.cjs', '.ts', '.py', '.go', '.java', '.md', '.yml', '.yaml'].includes(
            path.extname(e.name)
          )
        )
          continue;
  ...
  for (const cat of standard.categories) {
    ...
    for (const check of cat.checks) {
      ...
      catResults.push({
        id: check.id,
        label: check.label,
        status: evidence.found ? 'compliant' : 'gap',
        evidence: evidence.found ? evidence.evidence : null,
      });
    }
    results.push({
      id: cat.id,
      ...
      compliance: Math.round(
        (catResults.filter((c) => c.status === 'compliant').length / catResults.length) * 100
      ),
    });
  }
...
runSkill('compliance-officer', () => {
  ...
  const overallScore = Math.round(
    audits.reduce((s, a) => s + a.complianceScore, 0) / audits.length
  );
  const gaps = audits.flatMap((a) =>
    a.categories.flatMap((c) =>
      c.checks
        .filter((ch) => ch.status === 'gap')
        .map((ch) => ({ standard: a.standard, control: ch.id, label: ch.label }))
    )
  );

  const result = {
    directory: targetDir,
    standardsAudited: standards,
    ...
    recommendations: gaps
      .slice(0, 5)
      .map((g) => `[${g.standard}] ${g.control}: Implement ${g.label}`),
  };
