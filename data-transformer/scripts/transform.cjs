...
const argv = createStandardYargs()
  ...
  .option('out', { alias: 'o', type: 'string', description: 'Output file path (optional)' }).argv;
...
runSkill('data-transformer', () => {
  ...
  try {
    ...
    else if (argv.input.endsWith('.csv'))
      data = Papa.parse(content, { header: true, dynamicTyping: true }).data;
